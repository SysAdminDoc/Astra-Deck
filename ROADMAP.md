...
- [~] Build category-driven settings panel with search, profile badges, and diagnostics _(read-side in v4.23.0 + interactive boolean editing in v4.24.0 + search integration in v4.25.0. The existing `#q` filter drives both the QUICK_TOGGLES list and the schema overview; matching categories force-expand and the per-key sub-list filters to just the matches. ~264 boolean keys are now editable and searchable from the popup; non-boolean type editors + profile-badge integration are pending.)_
