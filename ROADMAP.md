...
- [ ] P3 — Start burning down the 277 light-theme-blind surfaces
  ...
  Note (2026-08-20, root cause found before burning anything down): a large share of the accepted surfaces **cannot** be given a light lane as they stand, because their colours do not come from the palette at all. Twenty `--ytkit-*` tokens were referenced under names nothing defined, so every one of those `var()` calls resolved to a fallback hex written beside it — the surface is hardwired and no palette or theme change can reach it. Ten were genuinely runtime-set and fine; six are now defined with their exact current values (zero rendering change) and `tests/ytkit-token-definitions.test.js` stops new ones appearing. **Define the remaining four first, then the light lanes become a palette edit instead of 254 hand-written rules.** They are listed in the separate item below because each needs a per-site decision. Related: the first surfaces examined (`ytkit-video-hide-btn`, the blocked-watch dialog) turned out to be gate false positives — they paint light text on their OWN opaque dark background, which is readable on either theme, so a burndown that just adds `html:not([dark])` rules to satisfy a source-text regex would be theatre. That is exactly what this item's third acceptance clause (a render lane, not a source rule) is guarding against.

- [ ] P3 — Define the three remaining divergent `--ytkit-*` tokens
  Why: `--ytkit-surface-elevated`, `--ytkit-surface-hover`, and `--ytkit-surface-raised` are referenced but never defined, and their call sites disagree about the fallback, so no single definition is a no-op. Until they exist, every surface using them is hardwired to a literal and cannot be themed — which blocks the light-theme burndown above. (`--ytkit-accent-contrast` was the fourth and is done: it was a real WCAG failure, white on the accent at 2.82:1, now dark at 6.32:1 and pinned against every built-in accent.)
  Evidence: `--ytkit-surface-raised` to `rgba(255, 255, 255, 0.04)` (3), `rgba(15, 23, 42, 0.04)` (3) and `#242a35`; `--ytkit-surface-elevated` to `#171b23` (2) and `rgba(23,27,35,0.96)`; `--ytkit-surface-hover` to `rgba(255,255,255,0.1)` and `#303746`. The divergent list is pinned in `tests/ytkit-token-definitions.test.js` and must only shrink.
  Touches: `extension/ytkit.js` (the `:root` palette block), `extension/features/video-hider/`, `extension/features/sticky-chat/`, `extension/features/subscription-groups/`, `tests/ytkit-token-definitions.test.js`
  Acceptance: each token is defined once, every call site that wanted a different value either loses its fallback or gains its own token, and the change in rendering at each site is stated explicitly rather than absorbed silently; `npm run smoke:a11y` passes and the divergent list in the test shrinks to empty.
  Complexity: M
