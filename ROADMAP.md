...
- [ ] P2 — SponsorBlock remaining-time adjustment
  Why: competitor userscript displays remaining time adjusted for SponsorBlock skips + playback speed. Astra Deck has `remainingTimeDisplay` and `sponsorBlock` but does not subtract skippable segment duration from the remaining-time calculation. Power users who use both features see inaccurate remaining time.
  Evidence: competitor userscript feature comparison (greasyfork.org/en/scripts/521686); Astra Deck `remainingTimeDisplay` at `extension/ytkit.js` computes `duration - currentTime` without SponsorBlock segment awareness.
