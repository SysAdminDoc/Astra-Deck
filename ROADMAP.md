...
Net: 7 new `extension/core/` modules, 4 feature carve-outs covering 10 monolith feature blocks, 2 user-visible popup surfaces (data-flow panel + schema category overview), 1 popup `Privacy` quick-toggle group, schema-driven risk badges on every applicable toggle row, userscript bundles the v5.0.0 core surface, +106 hardening tests (315 → 421), 18 versions shipped, every `npm run check` + `npm test` + `node build-extension.js` pipeline green. Open question (Ctrl+Shift+Y) resolved.
...
- [~] Build category-driven settings panel with search, profile badges, and diagnostics _(category roll-up read-side delivered in v4.23.0 — popup ships a collapsible `<details>` surface enumerating every settings-schema category with live enabled/total counts. Editing toggles per-category is still in the existing quick-toggle list; full schema-driven editing surface remains the next major slice.)_
