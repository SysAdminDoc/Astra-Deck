...
1. **Versioned selector packs** — [~] batches 1-2 shipped (v4.31.0 batch 1: appShell, nav+masthead, search, leftNav; v4.32.0 batch 2: feed, feedCard, thumbnail, shortsShelf). Pack registry drives `SurfaceSelectorMap`; 19 inline surfaces remain. Next batches: watch-shell (watch / relatedSidebar / player / mainVideo), player-chrome (playerChrome / playerSettings), engagement (comments / commentComposer / engagementPanels / modals), misc (settingsOverlay / profile / channelProfile / notifications / media / sidebar), live-chat (liveChatFrame / liveChat / liveChatPlaceholder).
