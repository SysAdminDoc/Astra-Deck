...
- [ ] P1 — In-Shorts player controls
  ...
  Where: new `extension/features/shorts-controls/`, selector pack additions for the Shorts player.
  Acceptance: On /shorts/ — visible seekbar with scrubbing, volume slider (persisted with rememberVolume), speed control honoring persistentSpeed, stop-looping toggle, optional auto-advance; each independently toggleable, off by default.
  Complexity: M
...
- [ ] P1 — Remaining-time display v2 (speed + SponsorBlock aware)
  Why: Existing remainingTimeDisplay ignores playbackRate and SB skips; Alchemy's version subtracts both — strictly better math on a feature Astra already ships.
  Evidence: YouTube Alchemy v11 remaining-time feature.
...
- [ ] P1 — Auto-exit fullscreen at video end (playlist-aware)
  ...
  Evidence: Alchemy; xdpirate (GF); CY Fung playlist-aware mod.
...
- [ ] P2 — Live-edge speed reset
  Why: With persistentSpeed >1x on a live stream, playback hits the live edge and stalls; Alchemy auto-resets to 1x when caught up.
  Evidence: Alchemy v10.16.
...
- [ ] P2 — Subtitles: auto-enable while muted + flash-on-rewind
  Why: Contextual caption triggers (muted → captions on; rewind → captions for 10s) are smart defaults Alchemy shipped in v11; complements autoSubtitles.
  Evidence: Alchemy v11.0.
...
- [ ] P2 — Guide sidebar per-item hiding
  Why: sidebarOrder reorders but cannot hide individual You/Explore entries (History, Playlists, Trending, Music, ...); Alchemy v11 does per-item granularity.
  Evidence: Alchemy v11.0 guide-sidebar controls.
...
- [ ] P2 — Notification menu controls: cap count + hide read
  ...
  Evidence: Alchemy notification options.
...
- [ ] P2 — Transcript modern-panel fallback pinning
  Why: YouTube is rolling out a modern transcript panel (data-target-id PAmodern_transcript_view); Alchemy v11.3-11.4 supports both variants — Astra's TranscriptService should too before it breaks.
  Evidence: Alchemy v11.4 changelog.
...
- [ ] P2 — Per-page CSS injection scoping
  Why: Most feature CSS is injected globally; Alchemy v11 cut perf cost by toggling CSS per page type. Astra's style lifecycle specs can carry a page-scope field.
  Evidence: Alchemy v11.0 per-page CSS toggling; `extension/core/styles.js` lifecycle specs.
