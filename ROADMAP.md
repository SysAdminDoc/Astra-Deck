...
- [ ] P3 — Reduce monolith compile time, the last large startup cost
  ...
  Found it (2026-08-19): the "rarely-used feature bodies" are not scattered - they are 15 inline FALLBACK objects. Every peeled feature is wired as `(globalThis.YTKitFeatures?.x?.createXFeature?.({...}) || { ...the entire pre-peel implementation, still inline... })`, so the monolith carries a second full copy of code that also ships as a module. Ten of the fifteen are full copies; five are already minimal stubs (360-551 bytes), which is the shape the other ten should take.
    ...
    Progress (2026-08-19): `sticky-video` done - 316,911 bytes removed, ytkit.js 3,204,260 -> 2,888,530 (-9.9%). Measured on an idle machine, three warm runs each side: monolith stage watch 69.4 -> 65.9 ms, feed 64.7 -> 60.8 ms. So ~10% of the file bought ~5% of the stage; the remaining nine should be worth roughly 10-14 ms in total, and the maintenance win is the larger half. Cost was 17 tests: behaviour assertions re-pointed at the module, two pure drift guards retired, one new test pinning the fallback under 2 KB so it cannot grow back. Use acorn to find the range - a brace matcher that does not skip regex literals over-runs badly.
    Do NOT batch these. `subscription-groups` was attempted next and reverted: removing its 164,457-byte fallback broke 34 tests across 11 files, and unlike sticky-video they are not all source pins - `npm run audit:overlays -- --self-test` fails too, because the audit tool scans ytkit.js for overlay patterns that lived in the fallback. Several count-based assertions ("both ytkit.js compact-count methods", "all three monolith CSV escapers") also need re-deriving rather than re-pointing. It needs its own session.
    Suggested order by blast radius, smallest first: `youtube-music-compat` (1,827 B), `return-dislike` (17,568 B), `video-notes` (17,911 B), `digital-wellbeing` (21,641 B), `dearrow` (29,995 B), `floating-logo` (31,264 B, monolith-only - no module to fall back to, so this one needs a real peel first), `sponsorblock` (35,016 B), `video-hider` (125,774 B), `subscription-groups` (164,457 B).
    Plan: replace each full fallback with the minimal descriptor stub the other five already use (id, name, description, group, icon, pages, no-op init/destroy). One feature per commit, `npm run check:startup` before and after on an idle machine, `next-monolith-peel.test.js` pins rewritten per feature. Start with `sticky-video`: largest, 100% duplicated, and the delegation site is a clean single expression.
