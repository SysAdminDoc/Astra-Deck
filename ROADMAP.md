...
1. **Versioned selector packs** — [~] batches 1-4 shipped (v4.31.0 batch 1: appShell, nav+masthead, search, leftNav; v4.32.0 batch 2: feed, feedCard, thumbnail, shortsShelf; v4.33.0 batch 3: watch, relatedSidebar, player, mainVideo; v4.34.0 batch 4: playerChrome, playerSettings, sidebar, modals). Pack registry drives `SurfaceSelectorMap`; 11 inline surfaces remain. Next batches: engagement (comments / commentComposer / engagementPanels), misc (settingsOverlay / profile / channelProfile / notifications / media), live-chat (liveChatFrame / liveChat / liveChatPlaceholder).
