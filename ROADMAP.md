...
- [ ] P3 — Search within playlist
  ...
  Evidence: Tweaks for YouTube features page; competitor userscript also implements playlist search.
...
- [ ] P3 — Color-coded video age borders on feed cards
  Why: competitor userscript implements color-coded borders indicating video age (fresh/week/month/year). Astra Deck has `videoAgeColors` (v3.9.0) but it uses CSS pseudo-elements. The competitor userscript approach of highlighting the last uploaded video on the subscriptions page with auto-scroll is a UX improvement.
  Evidence: competitor userscript README; Astra Deck `videoAgeColors` already exists but could be enhanced.
