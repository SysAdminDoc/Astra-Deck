...
1. **Versioned selector packs** — [~] batch 1 shipped in v4.31.0 (appShell, nav+masthead, search, leftNav split into `core/selector-packs/<surface>.js` with `captureEvidence` + `lastVerified`). Pack registry now drives `SurfaceSelectorMap`; remaining 23 inline surfaces migrate batch-by-batch through v5.1.x. Next batches: feed-shell (feed / feedCard / thumbnail / shortsShelf), watch-shell (watch / relatedSidebar / player / mainVideo), player-chrome (playerChrome / playerSettings), engagement (comments / commentComposer / engagementPanels / modals), misc (settingsOverlay / profile / channelProfile / notifications / media / sidebar), live-chat (liveChatFrame / liveChat / liveChatPlaceholder).
...
- Full competitive feature parity validation against UnTrap, Enhancer for YouTube, Improve YouTube, competitor userscript, YouTube Tweaks, Tweaks for YouTube, Unhook, BlockTube, PocketTube, SponsorBlock, DeArrow, Return YouTube Dislike, and Video Speed Controller.
...
| 14 | competitor userscript | Tim Macy | Userscript, Chrome, Firefox, Edge | Greasy Fork: 1,845 installs, v10.13, updated 2026-05-07. AMO: v10.13, updated 2026-05-07. GitHub: 86 stars, updated 2026-05-19. | 200+ | Native-feeling tab view, transcript export, header links, color-coded videos, dense UI settings. | https://greasyfork.org/en/scripts/521686-competitor userscript ; https://github.com/TimMacy/competitor userscript ; https://addons.mozilla.org/en-CA/firefox/addon/competitor userscript/ |
...
| Theming/UI | Dense/compact layout | Y | P | Y | Y | P | - | - | - | - | - | Y | Y | P | - | Y | competitor userscript has native-feeling compact tab layout. |
...
| Theming/UI | Watch tabs for info/comments/videos/transcript | Y | - | P | - | - | - | - | - | - | - | Y | - | Y | - | Y | competitor userscript tab view is the benchmark. |
...
| Player UI | Cinema/theater/full-window mode | Y | Y | Y | Y | - | - | - | - | - | - | Y | Y | Y | - | Y | Enhancer/competitor userscript are key baselines. |
...
| Player UI | Persistent progress bar/chapters | Y | P | Y | P | - | - | - | SB marks | - | - | Y | Y | Y | - | Y | competitor userscript with SponsorBlock chapter markers is baseline. |
...
| Media/downloads | Subtitle/transcript download | Y | - | P | Y | - | - | - | - | - | - | Y | P | Y | - | Y | competitor userscript transcript exporter is benchmark. |
...
| Navigation | Quick links/header links | Y | - | P | - | - | - | - | - | - | - | Y | - | - | - | Y | competitor userscript supports up to 10 header links. |
...
| Research/AI | Transcript handoff to ChatGPT/NotebookLM/etc. | Y | - | - | P | - | - | - | - | - | - | Y | - | - | - | Y | competitor userscript's header buttons are benchmark. |
...
- Research workspace: competitor userscript has transcript export; UnTrap/Rovetify have AI summaries; YouFocus has notes/time tracking. Astra should unify transcripts, notes, summaries, spaced review, quote clipping, Markdown export, and local search.
...
- [~] Add selector health system and versioned selector packs _(read-side delivered in v4.8.0 — `extension/core/selector-health.js` exports `createSelectorHealth()` + `summarizeSelectorHealth` + `rankSelectorProblems` + `formatSelectorCopyReport`. v4.31.0 begins the per-surface split — `extension/core/selector-packs/{appShell,nav,search,leftNav}.js` carry their own captureEvidence + lastVerified; SurfaceSelectorMap is built from `INLINE_SURFACES` + `SurfacePackRegistry`. Remaining 23 inline surfaces migrate batch-by-batch through v5.1.0.)_
...
- [~] Selector fixtures fail loudly when stable selectors disappear _(infra exists in `tests/selector-regression.test.js`. v4.31.0 begins the per-pack split — `extension/core/selector-packs/{appShell,nav,search,leftNav}.js` shipped with capture provenance + lastVerified. Remaining 23 inline surfaces migrate in v5.1.0+.)_
...
Goal: beat Enhancer for YouTube, Video Speed Controller, Tweaks for YouTube, YouTube HD, and competitor userscript in player control without adding keyboard shortcuts.
...
Goal: combine competitor userscript, Rovetify, UnTrap AI, and YouFocus into a local-first research workspace.
...
- Astra replaces at least these stacks for a power user: SponsorBlock, DeArrow, Return YouTube Dislike, Unhook, BlockTube, PocketTube, Video Speed Controller, competitor userscript, YouTube Tweaks, Tweaks for YouTube, and common Stylus dark/compact themes.
