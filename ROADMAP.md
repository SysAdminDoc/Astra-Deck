...
- [ ] P3 — Define the four divergent `--ytkit-*` tokens
  Why: `--ytkit-accent-contrast`, `--ytkit-surface-elevated`, `--ytkit-surface-hover`, and `--ytkit-surface-raised` are referenced but never defined, and their call sites disagree about the fallback, so no single definition is a no-op. Until they exist, every surface using them is hardwired to a literal and cannot be themed — which blocks the light-theme burndown above.
  Evidence: `--ytkit-accent-contrast` falls back to `#fff` (4 sites), `#14181f` and `#111318`; `--ytkit-surface-raised` to `rgba(255, 255, 255, 0.04)` (3), `rgba(15, 23, 42, 0.04)` (3) and `#242a35`; `--ytkit-surface-elevated` to `#171b23` (2) and `rgba(23,27,35,0.96)`; `--ytkit-surface-hover` to `rgba(255,255,255,0.1)` and `#303746`. The divergent list is pinned in `tests/ytkit-token-definitions.test.js` and must only shrink.
