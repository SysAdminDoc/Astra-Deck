...
1. **Versioned selector packs** — [~] batches 1-6 shipped (v4.31.0 batch 1: appShell, nav+masthead, search, leftNav; v4.32.0 batch 2: feed, feedCard, thumbnail, shortsShelf; v4.33.0 batch 3: watch, relatedSidebar, player, mainVideo; v4.34.0 batch 4: playerChrome, playerSettings, sidebar, modals; v4.35.0 batch 5: comments, commentComposer, engagementPanels; v4.36.0 batch 6: settingsOverlay, profile+channelProfile, notifications, media). Pack registry drives `SurfaceSelectorMap`; 3 inline surfaces remain (the live-chat trio). Final batch: live-chat (liveChatFrame / liveChat / liveChatPlaceholder).
