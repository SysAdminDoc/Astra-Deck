...
1. **Versioned selector packs** — [~] batches 1-3 shipped (v4.31.0 batch 1: appShell, nav+masthead, search, leftNav; v4.32.0 batch 2: feed, feedCard, thumbnail, shortsShelf; v4.33.0 batch 3: watch, relatedSidebar, player, mainVideo). Pack registry drives `SurfaceSelectorMap`; 15 inline surfaces remain. Next batches: player-chrome (playerChrome / playerSettings / sidebar), engagement (comments / commentComposer / engagementPanels / modals), misc (settingsOverlay / profile / channelProfile / notifications / media), live-chat (liveChatFrame / liveChat / liveChatPlaceholder).
