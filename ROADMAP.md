...
- [x] **P3 / M — `hideVideosFromHome` channel-key cache (EI-NEW5)**: precompute
...
- [x] **P2 / XL — Top-3 monolith peel**: split `stickyVideo`,
  ...
  - [x] 2026-06-04: extracted the remaining `chatStyleComments` DOM observer,
    selection guard, comment normalization, reply-dialog styling, mutation
    scheduling, and teardown into the chat-style feature module with inline
    monolith fallback retained.
  - [x] Split `stickyVideo` into a feature module.
    ...
    - [x] 2026-06-04: extracted the remaining `stickyVideo` runtime/state
...
2. **Continue feature peels (long tail)** — [~] +11 peels this session (v4.38.0 wave-8 CSS quintet → `features/wave-8-css/`; v4.43.0 home-subs CSS sextet → `features/home-subs-css/`). Total peeled: 21 of ~200 monolith blocks. v5.2.0+ scope: continue batch-by-batch (next likely batches are wave-10 competitor userscript CSS, watch-page CSS, and comments CSS clusters). DOM-walking observers are blocked on slice #3 and remain v5.2.0+ work.
