...
// @version      7.12.1
// @namespace    TimMacy.competitor userscript
...
// @noframes
// @homepageURL  https://github.com/TimMacy/competitor userscript
// @supportURL   https://github.com/TimMacy/competitor userscript/issues
// @downloadURL https://update.greasyfork.org/scripts/521686/YouTube%20competitor userscript.user.js
// @updateURL https://update.greasyfork.org/scripts/521686/YouTube%20competitor userscript.meta.js
// ==/UserScript==
...
*                    Version: 7.12.1 - competitor userscript                  *
*                                                                       *
...
(async function () {
    ...
    styleSheet.textContent = `
        ...
        .CentAnni-header:hover { color: white; }
        ...
        .CentAnni-copyright:hover { color: #369eff; }
        ...
        .checkbox-field { margin-right: 10px; cursor: pointer; }
        ...
        :is(:fullscreen, :-webkit-full-screen, .html5-video-player.ytp-fullscreen)
            :is(#CentAnni-progress-bar-start, #CentAnni-progress-bar-end) {
                width: 24px !important;
        }

        :is(:fullscreen, :-webkit-full-screen, .html5-video-player.ytp-fullscreen)
                :has(.CentAnni-remaining-time-container)
        ...
        :is(:fullscreen, :-webkit-full-screen, .html5-video-player.ytp-fullscreen)
                :has(.CentAnni-remaining-time-container)
                .ytp-autohide #ytp-caption-window-container {
                    bottom: 40px;
                    top: unset;
        }
        ...
        .CentAnni-progress-bar {
            ...
            #CentAnni-progress-bar-progress, #CentAnni-progress-bar-buffer {
                width: 100%;
                height: 3px;
            ...
            .ad-interrupting #CentAnni-progress-bar-progress { background: transparent; }
            .ytp-ad-persistent-progress-bar-container { display: none; }
            #CentAnni-progress-bar-buffer { background: rgba(255, 255, 255, .4); }

            .ytp-autohide #CentAnni-progress-bar-start.active,
            .ytp-autohide #CentAnni-progress-bar-bar.active,
            .ytp-autohide #CentAnni-progress-bar-end.active { opacity: 1; }
            ...
            #CentAnni-progress-bar-start, #CentAnni-progress-bar-end {
                position: absolute;
                height: 3px;
                width: 12px;
                bottom: 0;
        ...
        .CentAnni-video-tabView:fullscreen ytd-watch-flexy ytd-engagement-panel-section-list-renderer[target-id=engagement-panel-macro-markers-description-chapters] { top: 52px; }
        ...
        .CentAnni-style-square-design {
            ...
            .ytd-page-manager[page-subtype="home"] {
                ...
                .CentAnni-style-live-video, .CentAnni-style-upcoming-video, .CentAnni-style-newly-video, .CentAnni-style-recent-video, .CentAnni-style-lately-video, .CentAnni-style-latterly-video, .CentAnni-style-old-video { border-radius: 0; }
            }
        ...
        .ytd-page-manager[page-subtype="home"] {
            .CentAnni-style-live-video, .CentAnni-style-upcoming-video, .CentAnni-style-newly-video, .CentAnni-style-recent-video, .CentAnni-style-lately-video, .CentAnni-style-latterly-video { outline: 2px solid; border-radius: 12px; }
    ...
    (document.head
        ...
        if (head)
            ...
            console.error("competitor userscript: Failed to find head element. Using backup to append stylesheet.");
        }
    });
    ...
    try {
        ...
        console.error("competitor userscript: Error loading user configuration:", error);
    }
    ...
    function showSettingsModal() {
        const existingModal = document.getElementById('yt-competitor userscript-settings-modal');
        if (existingModal) {
        ...
        modal.id = 'yt-competitor userscript-settings-modal';
        ...
        header.href = 'https://github.com/TimMacy/competitor userscript';
        header.target = '_blank';
        header.rel = 'noopener';
        header.textContent = 'competitor userscript';
        header.title = 'GitHub Repository for competitor userscript';
        ...
        resetButton.onclick = async () => {
            ...
            try {
                ...
                window.closecompetitor userscriptSettingsModal();
                setTimeout(() => { location.reload(); }, 900);
            } catch (error) {
                showNotification('Error resetting settings to default!');
                console.error("competitor userscript: Error resetting settings to default:", error);
            }
        };
        ...
        cancelButton.onclick = () => { window.closecompetitor userscriptSettingsModal(); };
        ...
        document.querySelector('.chatgpt-prompt-textarea').addEventListener('click', function () {
            ...
            if (contentWillFit) {
                desiredScrollTop = contentHeightAfterExpansion - modalVisibleHeight;
        ...
        function setupModalHandlers(modal) {
            ...
            function closeModalOverlayClickHandler(event) {
                const mainModal = document.getElementById('yt-competitor userscript-settings-modal');
            ...
            const escKeyListener = function (event) {
                if (event.key === 'Escape' && event.type === 'keydown') {
                    ...
                    if (openSubPanel) {
                        ...
                        const modal = document.getElementById('yt-competitor userscript-settings-modal');
                        if (modal) window.closecompetitor userscriptSettingsModal();
                    }
                }
            };
    ...
    async function saveSettings() {
        ...
        try {
            ...
            console.error("competitor userscript: Error saving user configuration:", error);
        }
    }
    ...
    async function exportSettings() {
        try {
            ...
            a.href = url;
            a.download = `competitor userscript_v${scriptVersion}_Backup_${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
            ...
            console.error("competitor userscript: Error exporting user settings:", error);
        }
    }
    ...
    function addButton() {
        ...
        const buttons = [
            { id: 'transcript-settings-button', text: USER_CONFIG.buttonIcons.settings, clickHandler: showSettingsModal, tooltip: 'competitor userscript Settings', ariaLabel: 'competitor userscript Settings.' },
    ...
    function addSettingsButton() {
        document.querySelectorAll('.CentAnni-button-wrapper').forEach(el => el.remove());
        const buttons = [{ id: 'transcript-settings-button', text: USER_CONFIG.buttonIcons.settings, clickHandler: showSettingsModal, tooltip: 'competitor userscript Settings', ariaLabel: 'competitor userscript Settings.' },];
    ...
    function handleTranscriptAction(callback) {
        ...
        if (!transcriptButton && !transcriptSection) {
            console.error("competitor userscript: Transcript button or section not found. Subtitles or closed captions are unavailable or language is unsupported. Reload this page to try again.");
        ...
        if (transcriptItems.length === 0) {
            console.error("competitor userscript: Transcript has not loaded.");
    ...
    function getTranscriptText() {
        ...
        if (!transcriptContainer) {
            console.error("competitor userscript: Transcript container not found.");
            return '';
        }
    ...
    function preLoadTranscript() {
        return new Promise((resolve, reject) => {
            ...
            const fallbackTimer = setTimeout(() => {
                if (!loaded) {
                    console.error("competitor userscript: The transcript took too long to load. Reload this page to try again.");
                    cleanup(true);
                    reject();
                }
            }, 10000);
    ...
    async function setTrackLanguage(categorySelector, languagePattern) {
        ...
        if (!categoryItem) { console.error('competitor userscript Category not found'); return; }
        categoryItem.click();
        ...
        if (!target) { console.error('competitor userscript Language not found'); return; }
        target.click();
                ...
                if (chapters.length !== previousChaptersLength || !cachedMaskImage) {
                    ...
                    if (chapters.length) {
                        ...
                        chapters.forEach((chapter) => {
                            ...
                            if (Number.isFinite(startPerc) && Number.isFinite(widthPerc))
                ...
                [thumbnailLink, titleLink].forEach(link => {
                    if (link && link.href && (link.href.includes('list=WL') || link.href.includes('list=PL') || link.href.includes('list=LL'))) {
                        try {
                            ...
                            console.error('competitor userscript: Error processing link:', e);
                        }
                    }
                });
            });
    ...
    function handleYTNavigation() {
        ...
        document.querySelectorAll('.CentAnni-button-wrapper:not(:has(#transcript-settings-button)), #CentAnni-channel-btn, .CentAnni-remaining-time-container, .CentAnni-info-date, #CentAnni-progress-bar-bar, #CentAnni-progress-bar-start, #CentAnni-progress-bar-end, #yt-competitor userscript-settings-modal').forEach(el => el.remove());
    }
    ...
    function runSetVideoQuality() {
        ...
        const policy = window.trustedTypes && trustedTypes.createPolicy('CentAnnicompetitor userscript', { createScript: s => s });
    ...
    // initiate the script
    async function initializecompetitor userscript() {
    ...
    function handleYouTubeNavigation() {
        // console.log("competitor userscript: Event Listner Arrived");
        const newURL = window.location.href;
        if (newURL !== currentURL) {
            currentURL = newURL;
            // console.log("competitor userscript: Only One Survived");
            ...
            const init = () => { if (pageObserver) { pageObserver.disconnect(); pageObserver = null; } requestIdleCallback(initializecompetitor userscript, { timeout: 2500 }); };
