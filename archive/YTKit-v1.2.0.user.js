...
(function() {
    ...
    const settingsManager = {
        defaults: {
            ...
            cfShowChapterHUD: true,         // show chapter name overlay on video player
        },
    ...
    const features = [
        ...
        // ═══════════════════════════════════════════════════════════════════
        {
            ...
            _CF_PROVIDERS: {
                ...
                browserai:  { name: 'Browser AI (local LLM)', endpoint: null, needsKey: false, defaultModel: null },
            ...
            _CF_BROWSER_AI_MODELS: {
                'SmolLM2-360M-Instruct':  { id: 'onnx-community/SmolLM2-360M-Instruct', size: '~200MB', speed: 'Fast', quality: 'Good' },
                'Qwen2.5-0.5B-Instruct':  { id: 'onnx-community/Qwen2.5-0.5B-Instruct', size: '~300MB', speed: 'Medium', quality: 'Better' },
                'Llama-3.2-1B-Instruct':  { id: 'onnx-community/Llama-3.2-1B-Instruct', size: '~700MB', speed: 'Slower', quality: 'Best' },
            },
            ...
            async _generateSummary() {
                ...
                if (!segments?.length) { if (btn) { btn.disabled = false; btn.textContent = 'Summarize'; } this._updateStatus('No transcript', 'error', 0); return; }
    
                const provider = appState.settings.cfLlmProvider || 'builtin';
                let summary;
                const summarySystemPrompt = `You are an expert video content summarizer. Given a timestamped transcript, write a clear, informative summary.

Structure your summary as:
1. **Opening sentence**: What this video is about and who made it (if evident)
2. **Key points** (2-3 sentences): The most important information, arguments, or findings presented
3. **Conclusion/Takeaway** (1 sentence): The main actionable insight or conclusion

Rules:
- Write 4-6 sentences total, 100-200 words
- Be specific — use names, numbers, and concrete details from the transcript
- Don't start with "This video" or "In this video" — lead with the actual content
...
- Output ONLY the summary text, no headers or formatting`;
                ...
                } else if (provider === 'browserai') {
                    try {
                        const transcriptText = this._buildTranscriptText(segments, 4000);
                        const rawText = await this._callBrowserAI(
                            summarySystemPrompt,
                            `Summarize this video transcript:\n\n${transcriptText}`,
                            (t, s, p) => this._updateStatus(t, s, p));
                        summary = rawText?.trim();
                    } catch(e) {
                        this._log('Browser AI summary error:', e.message);
                        this._updateStatus('Browser AI failed, using extractive fallback', 'loading', 80);
                        summary = _extractiveSummary(segments);
                    }
                } else {
                    try {
                        ...
                        if (segments?.length) {
                            ...
                            if (this._liveAccumulated.length >= 10 && now - this._liveLastGenTime > 120000) {
                                ...
                                if (provider === 'builtin') {
                                    ...
                                    try {
                                        const transcriptText = this._buildTranscriptText(this._liveAccumulated, 4000);
                                        const durationMin = Math.ceil(duration / 60);
                                        const systemPrompt = this._buildLiveChapterSystemPrompt(durationMin);
                                        const rawText = await this._callBrowserAI(systemPrompt, `Generate chapters for this live stream transcript:\n\n${transcriptText}`);
                                        data = this._parseChapterJSON(rawText, duration);
                                    } catch(e) {
                                        this._log('Live: Browser AI failed, falling back to heuristic:', e.message);
                                        data = this._generateChaptersHeuristic(this._liveAccumulated, duration);
                                    }
                                } else {
                                    try {
                    ...
                    try {
                        ...
                        if (segments?.length) {
                            ...
                            if (provider === 'builtin') {
                                ...
                                try {
                                    const transcriptText = this._buildTranscriptText(segments, 4000);
                                    const systemPrompt = this._buildChapterSystemPrompt(0);
                                    const rawText = await this._callBrowserAI(systemPrompt, `Generate chapters:\n\n${transcriptText}`);
...
Rules: start in seconds (integers), first at 0, 3-8 chapters, 2-6 POIs. Titles 3-7 words, specific to content discussed. Output ONLY JSON.`;
            ...
            async _generateChapters(videoId, onStatus) {
                ...
                try {
                    ...
                    if (provider === 'builtin') {
                        ...
                        onStatus?.('Preparing transcript for Browser AI...', 'loading', 55);
                        const transcriptText = this._buildTranscriptText(segments, 4000);
                        const durationMin = Math.ceil(duration / 60);
                        const systemPrompt = this._buildChapterSystemPrompt(durationMin);
                        const userPrompt = `Generate chapters and points of interest for this video transcript:\n\n${transcriptText}`;
                        const rawText = await this._callBrowserAI(systemPrompt, userPrompt, onStatus);
                        onStatus?.('Parsing AI results...', 'loading', 95);
                        data = this._parseChapterJSON(rawText, duration);
                        if (!data?.chapters?.length) {
                            this._log('Raw Browser AI output:', rawText);
                            onStatus?.('Browser AI could not generate valid chapters — try a larger model or use Built-in', 'error', 0);
                            this._isGenerating = false; return null;
                        }
                    } else {
                        const endpoint = this._getLlmEndpoint();
            ...
            _updateChapterHUD(chapterIdx) {
                ...
                const ch = this._chapterData.chapters[chapterIdx];
                const color = this._CF_COLORS[chapterIdx % this._CF_COLORS.length];
                const nextIdx = chapterIdx + 1;
                const nextCh = this._chapterData.chapters[nextIdx];
                const nextTitle = nextCh ? nextCh.title : '';
    
                let html = `<span class="cf-hud-dot" style="background:${color}"></span>`;
                html += `<span class="cf-hud-title">${ch.title}</span>`;
                if (nextTitle) {
                    html += `<span class="cf-hud-next">Next: ${nextTitle}</span>`;
                }
            ...
            _renderPanel() {
                ...
                if (this._activeTab === 'chapters') {
                    ...
                    if (this._lastSummary) {
                        tabHTML += `<div class="cf-summary-box">${this._lastSummary}</div><button class="cf-action-btn" id="cf-copy-summary" style="margin-top:4px">Copy Summary</button>`;
                    }
                    ...
                    const showApiFields = s.cfLlmProvider && s.cfLlmProvider !== 'builtin' && s.cfLlmProvider !== 'browserai';
                    const showKeyField = currentProvider.needsKey || s.cfLlmProvider === 'custom';
                    const showEndpointField = s.cfLlmProvider === 'custom';
                    const showBrowserAiFields = s.cfLlmProvider === 'browserai';
                    const showOllamaManager = s.cfLlmProvider === 'ollama';
                    const browserAiModelOptions = Object.entries(this._CF_BROWSER_AI_MODELS).map(([k, v]) => `<option value="${k}" ${s.cfBrowserAiModel === k ? 'selected' : ''}>${k} (${v.speed}, ${v.size})</option>`).join('');
                    ...
                    tabHTML = `
                        ...
                        ${showBrowserAiFields ? `
                            <div class="cf-settings-row"><span class="cf-settings-label">Browser AI Model</span><select class="cf-select" id="cf-browserai-model-select">${browserAiModelOptions}</select></div>
                            <div style="font-size:10px;color:rgba(124,58,237,0.4);margin:-2px 0 8px;padding-left:2px">Runs locally via WebGPU (Chrome/Edge). Model downloads once, then cached by browser.</div>
                        ` : ''}
                        ${showApiFields ? `
                        ...
                        ` : (!showBrowserAiFields && !showOllamaManager ? '<div style="font-size:10px;color:rgba(255,255,255,0.15);margin:-2px 0 8px;padding-left:2px">Analyzes transcript patterns locally — NLP engine with TF-IDF topic segmentation.</div>' : '')}
                        ${ollamaHTML}
                ...
                bindSelect('#cf-browserai-model-select', 'cfBrowserAiModel');
                    ...
                    if (result.running) {
                        ...
                        if (installedEl && result.models.length) {
                            const modelHTML = result.models.map(m => {
                                const isCurrent = (appState.settings.cfLlmModel || '').includes(m.replace(':latest',''));
                            ...
                            installedEl.querySelectorAll('.cf-ollama-use').forEach(btn => {
                                btn.addEventListener('click', () => {
                                    appState.settings.cfLlmModel = btn.dataset.cfModel;
                                    settingsManager.save(appState.settings);
                                    self._renderPanel();
                                });
                            });
                ...
                this._panelEl?.querySelectorAll('.cf-ollama-pull').forEach(btn => {
                    btn.addEventListener('click', async () => {
                        ...
                        try {
                            ...
                            appState.settings.cfLlmModel = model;
                            settingsManager.save(appState.settings);
                        } catch(e) {
            ...
            init() {
                const css = `
                    ...
                    .cf-chapter-hud { position:absolute;top:12px;left:12px;display:flex;align-items:center;gap:8px;padding:6px 14px 6px 10px;background:rgba(10,10,15,0.75);border-radius:8px;border:1px solid color-mix(in srgb, var(--cf-hud-accent, #7c3aed) 30%, transparent);backdrop-filter:blur(12px);z-index:60;pointer-events:none;opacity:0;transition:opacity 0.3s;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;box-shadow:0 4px 20px rgba(0,0,0,0.4); }
                    ...
                    .cf-hud-title { font-size:13px;font-weight:600;color:#e0e0e8;letter-spacing:0.2px; }
                    .cf-hud-next { font-size:10px;color:rgba(255,255,255,0.3);margin-left:4px;font-weight:400; }
        ...
        },
        {
            ...
            group: 'ChapterForge', icon: 'settings-2', type: 'select', dependsOn: 'chapterForge',
            options: { 'builtin': 'Built-in (no setup)', 'browserai': 'Browser AI (local LLM)', 'openai': 'OpenAI', 'ollama': 'Ollama (local)', 'openrouter': 'OpenRouter', 'custom': 'Custom Endpoint' },
            init() {}, destroy() {}
        },
        {
...
`;
        ...
        if (lastSeenVer !== CURRENT_VER) {
            ...
            versionSpan.onclick = () => {
                ...
                badge.remove();
                showToast('v22.6: NLP engine + Ollama model manager', '#3b82f6', { duration: 6 });
            };
        }
