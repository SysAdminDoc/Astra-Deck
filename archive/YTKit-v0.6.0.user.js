    ...
    const features = [
        ...
        // ─── Appearance ───
        {
            ...
            init() {
                ...
                // Only re-apply on navigation (page transitions), not on every DOM mutation
                addNavigateRule(this._ruleId, () => this._applyTheme(theme));
            },

            _applyTheme(theme) {
                // Skip if theme is already applied
                if (document.getElementById('ytkit-theme-style')?.dataset.theme === theme) {
                    // Just ensure dark attribute stays set
                    if (theme !== 'system') document.documentElement.setAttribute('dark', '');
                    return;
                }
        ...
        },
        {
            ...
            init() {
                this._clickHandler = (e) => {
                    // Don't intercept modifier keys (open in new tab, etc.)
                    ...
                    // Skip blob/data URLs (used by file exports/downloads)
                    if (href.startsWith('blob:') || href.startsWith('data:')) return;

                    // Only intercept internal YouTube links
                    const isInternal = href.startsWith('/') || href.includes('youtube.com/');
                    // Skip hash-only links, javascript:, and external
                    const isNavigable = !href.startsWith('#') && !href.startsWith('javascript:');

                    if (isInternal && isNavigable) {
        ...
        // ─── Video Player ───
        {
            ...
            applyStyles() {
                ...
                if (isWatchPage) {
                    try { window.localStorage.setItem('yt-player-theater-mode', JSON.stringify({ data: 'true', creation: Date.now() })); } catch(e) {}
            ...
            init() {
                ...
                if (window.location.pathname.startsWith('/watch')) {
                    document.documentElement.classList.add('yt-suite-fit-to-window');
                    document.body.classList.add('yt-suite-fit-to-window');
                }
        ...
        },
        {
            ...
            _getSavedLayout() {
                ...
                // Fall back to legacy position-only key
                try {
            ...
            _initResize(container, handles) {
                ...
                const onMouseMove = (e) => {
                    ...
                    if (resizeState.corner === 'bl') {
                        // Bottom-left: width grows leftward, left edge moves
                        ...
                        // Bottom-right: width grows rightward
            ...
            init() {
                const preloadRule = () => {
                    ...
                    const tryPreload = (attempts = 0) => {
                        ...
                        // Temporarily position the continuation element in the viewport
                        // (invisible, no interaction) to trigger YouTube's IntersectionObserver
                        ...
                        // Give the IntersectionObserver a couple of frames to detect it, then restore
