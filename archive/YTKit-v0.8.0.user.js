...
(function() {
        ...
        _expose() {
            window.YTKit = {
                ...
                resetSettings: async () => {
                    ...
                    showToast('Settings reset — reloading...', '#f59e0b');
    ...
    const features = [
        ...
        cssFeature('hideNotificationBadge', 'Hide Notification Badge', 'Hide the red notification count badge', 'Interface', 'bell-minus',
            `ytd-notification-topbar-button-renderer .yt-spec-icon-badge-shape__badge { display: none !important; }`),
        ...
        },
        {
            ...
            description: 'Add full player controls (progress bar, speed, quality) to YouTube Shorts',
            group: 'Content',
            icon: 'film',
            _styleElement: null,
            _ruleId: 'shortsPlayerControls',
            _enhanceShorts() {
                const reels = document.querySelectorAll('ytd-reel-video-renderer');
                reels.forEach(reel => {
                    if (reel.dataset.ytkitEnhanced) return;
                    reel.dataset.ytkitEnhanced = '1';
                    const video = reel.querySelector('video');
                    if (!video) return;
                    video.controls = true;
                    video.style.cssText += 'object-fit: contain !important;';
                    // Remove loop so progress bar works
            ...
            init() {
                ...
                `;
                this._styleElement = injectStyle(css, this.id, true);
                addMutationRule(this._ruleId, () => this._enhanceShorts());
            },
            destroy() {
                this._styleElement?.remove();
                removeMutationRule(this._ruleId);
                document.querySelectorAll('[data-ytkit-enhanced]').forEach(el => {
        ...
        },
        {
            ...
            description: 'Show YouTube logo + settings gear on watch pages. Choose overlay (on player hover) or inline (below video).',
            group: 'Video Player',
            icon: 'youtube',
            isParent: true,
            ...
            _injectInline() {
                ...
                waitForElement('#top-row #owner', (ownerDiv) => {
                    ...
                    const gearBtn = document.createElement('button');
                    gearBtn.title = 'YTKit Settings';
                    gearBtn.style.cssText = 'background:none;border:none;cursor:pointer;padding:4px;display:flex;align-items:center;opacity:0.7;transition:opacity 0.2s;';
                    gearBtn.onmouseenter = () => { gearBtn.style.opacity = '1'; };
                    gearBtn.onmouseleave = () => { gearBtn.style.opacity = '0.7'; };
                    const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
                    svg.setAttribute('viewBox', '0 0 24 24');
                    ...
                    p.setAttribute('d', 'M12.22 2h-.44a2 2 0 00-2 2v.18a2 2 0 01-1 1.73l-.43.25a2 2 0 01-2 0l-.15-.08a2 2 0 00-2.73.73l-.22.38a2 2 0 00.73 2.73l.15.1a2 2 0 011 1.72v.51a2 2 0 01-1 1.74l-.15.09a2 2 0 00-.73 2.73l.22.38a2 2 0 002.73.73l.15-.08a2 2 0 012 0l.43.25a2 2 0 011 1.73V20a2 2 0 002 2h.44a2 2 0 002-2v-.18a2 2 0 011-1.73l.43-.25a2 2 0 012 0l.15.08a2 2 0 002.73-.73l.22-.39a2 2 0 00-.73-2.73l-.15-.08a2 2 0 01-1-1.74v-.5a2 2 0 011-1.74l.15-.09a2 2 0 00.73-2.73l-.22-.38a2 2 0 00-2.73-.73l-.15.08a2 2 0 01-2 0l-.43-.25a2 2 0 01-1-1.73V4a2 2 0 00-2-2z');
                    const c = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
                    c.setAttribute('cx', '12'); c.setAttribute('cy', '12'); c.setAttribute('r', '3');
                    svg.appendChild(p); svg.appendChild(c);
                    gearBtn.appendChild(svg);
                    gearBtn.addEventListener('click', (e) => {
                        e.stopPropagation();
                        document.body.classList.toggle('ytkit-panel-open');
                    });
        ...
        },
        {
            ...
            init() {
                addNavigateRule('theaterAutoScroll', () => {
                    if (!window.location.pathname.startsWith('/watch')) return;
            ...
            destroy() { removeNavigateRule('theaterAutoScroll'); }
        },
        cssFeature('persistentProgressBar', 'Always Show Progress Bar', 'Keep the video progress bar visible at all times', 'Video Player', 'align-horizontal-justify-center',
        ...
        },
        {
            id: 'abLoop',
            name: 'A-B Loop',
            description: 'Set custom loop start/end points for repeating a section of a video',
            group: 'Video Player',
            icon: 'repeat',
            _buttonId: 'ytkit-abloop-btn',
            _state: { a: null, b: null, active: false, videoId: null },
            _timeUpdateHandler: null,
            _updateButton(btn) {
                const s = this._state;
                if (s.a !== null && s.b !== null) {
                    btn.title = `Loop: ${s.a.toFixed(1)}s - ${s.b.toFixed(1)}s (click to clear)`;
                    btn.style.opacity = '1';
                    btn.style.color = '#ff6b6b';
                } else if (s.a !== null) {
                    btn.title = `A: ${s.a.toFixed(1)}s — click to set B point`;
                    btn.style.opacity = '1';
                    btn.style.color = '#fbbf24';
                } else {
                    btn.title = 'Set A point for A-B loop';
            ...
            _handleClick() {
                ...
                if (this._state.a !== null && this._state.b !== null) {
                    // Clear loop
                    this._state = { a: null, b: null, active: false, videoId: this._state.videoId };
                    video.removeEventListener('timeupdate', this._timeUpdateHandler);
                    showToast('Loop cleared', '#64748b');
                } else if (this._state.a === null) {
                    this._state.a = video.currentTime;
                    showToast(`A point: ${this._state.a.toFixed(1)}s`, '#fbbf24');
                } else {
                    this._state.b = video.currentTime;
                    if (this._state.b < this._state.a) [this._state.a, this._state.b] = [this._state.b, this._state.a];
                    this._state.active = true;
                    this._timeUpdateHandler = () => {
                        if (this._state.active && video.currentTime >= this._state.b) {
                            video.currentTime = this._state.a;
                        }
                    };
                    video.addEventListener('timeupdate', this._timeUpdateHandler);
                    showToast(`Looping ${this._state.a.toFixed(1)}s - ${this._state.b.toFixed(1)}s`, '#22c55e');
                }
                if (btn) this._updateButton(btn);
            },
            init() {
                registerPersistentButton(this._buttonId, '.ytp-right-controls', `#${this._buttonId}`, (parent) => {
                    const btn = document.createElement('button');
                    btn.id = this._buttonId;
                    btn.className = 'ytp-button';
                    TrustedHTML.setHTML(btn, `<svg height="100%" width="100%" viewBox="0 0 24 24"><path d="M17 2l4 4-4 4" fill="none" stroke="white" stroke-width="1.5"/><path d="M3 11V9a4 4 0 014-4h14M7 22l-4-4 4-4" fill="none" stroke="white" stroke-width="1.5"/><path d="M21 13v2a4 4 0 01-4 4H3" fill="none" stroke="white" stroke-width="1.5"/><text x="12" y="14" fill="white" font-size="7" text-anchor="middle" font-weight="bold">AB</text></svg>`);
                    btn.style.cssText = 'opacity:0.9;cursor:pointer;border:none;background:none;padding:0 4px;';
                    btn.addEventListener('click', (e) => { e.stopPropagation(); this._handleClick(); });
                    this._updateButton(btn);
                    parent.insertBefore(btn, parent.firstChild);
                });
                // Clear loop on video change
                addNavigateRule('abLoopClear', () => {
                    const newId = new URLSearchParams(window.location.search).get('v');
                    if (newId !== this._state.videoId) {
                        const video = document.querySelector('video.html5-main-video');
                        if (video && this._timeUpdateHandler) video.removeEventListener('timeupdate', this._timeUpdateHandler);
                        this._state = { a: null, b: null, active: false, videoId: newId };
                        const btn = document.getElementById(this._buttonId);
                        if (btn) this._updateButton(btn);
                    }
                });
            ...
            destroy() {
                unregisterPersistentButton(this._buttonId);
                removeNavigateRule('abLoopClear');
                const video = document.querySelector('video.html5-main-video');
                if (video && this._timeUpdateHandler) video.removeEventListener('timeupdate', this._timeUpdateHandler);
                this._state = { a: null, b: null, active: false, videoId: null };
            }
        },
        ...
        },
        {
            id: 'volumeBoost',
            name: 'Volume Boost',
            description: 'Amplify video volume beyond 100% using AudioContext (up to 600%)',
            group: 'Playback',
            icon: 'volume-2',
            _audioCtx: null,
            _gainNode: null,
            _source: null,
            _buttonId: 'ytkit-volboost-btn',
            _currentBoost: 100,
            _setupAudio(video) {
                ...
                try {
                    ...
                    const level = appState.settings.volumeBoostLevel || 200;
                    this._gainNode.gain.value = level / 100;
                    this._currentBoost = level;
                } catch (e) {
                    DebugManager.log('VolumeBoost', 'AudioContext setup failed', e.message);
                }
            },
            _setBoost(level) {
                if (this._gainNode) {
                    ...
                    this._currentBoost = level;
                    const btn = document.getElementById(this._buttonId);
                    if (btn) btn.title = `Volume: ${level}%`;
                    if (level > 100) showToast(`Volume: ${level}%`, '#22c55e');
                }
            },
            _cycleBoost() {
                const levels = [100, 150, 200, 300, 400, 600];
                const idx = levels.indexOf(this._currentBoost);
            ...
            init() {
                ...
                registerPersistentButton(this._buttonId, '.ytp-right-controls', `#${this._buttonId}`, (parent) => {
                    const video = document.querySelector('video.html5-main-video');
                    if (video) self._setupAudio(video);
                    const btn = document.createElement('button');
                    btn.id = self._buttonId;
                    btn.className = 'ytp-button';
                    btn.title = `Volume: ${self._currentBoost}%`;
                    TrustedHTML.setHTML(btn, `<svg height="100%" width="100%" viewBox="0 0 24 24"><polygon points="11,5 6,9 2,9 2,15 6,15 11,19" fill="none" stroke="white" stroke-width="1.5"/><path d="M15.54 8.46a5 5 0 010 7.07M19.07 4.93a10 10 0 010 14.14" fill="none" stroke="white" stroke-width="1.5"/></svg>`);
                    btn.style.cssText = 'opacity:0.9;cursor:pointer;border:none;background:none;padding:0 4px;';
                    btn.addEventListener('click', (e) => { e.stopPropagation(); self._cycleBoost(); });
                    parent.insertBefore(btn, parent.firstChild);
                });
            },
            destroy() {
                ...
                this._audioCtx = null; this._gainNode = null; this._source = null;
            }
        },
        {
            id: 'volumeBoostLevel',
            name: 'Max Volume Boost',
            description: 'Maximum boost level when cycling (100-600%)',
            group: 'Playback',
            icon: 'volume-2',
            isSubFeature: true,
            parentId: 'volumeBoost',
            type: 'range',
        ...
        },
        {
            ...
            init() {
                ...
                this._wheelHandler = (e) => {
                    ...
                    // Only if hovering over the player area, not over controls
        ...
        cssFeature('hideLatestPosts', 'Hide Latest Posts', 'Remove community posts and updates sections', 'Clutter', 'message-square-off',
            `ytd-post-renderer,
        ...
        },
        {
            ...
            _createPlayer(streamData) {
                ...
                this._persistenceObserver = new MutationObserver((mutations) => {
                    ...
                    if (!moviePlayer.classList.contains('ytkit-embed-active')) {
                        moviePlayer.classList.add('ytkit-embed-active');
                        DebugManager.log('Embed', 'Re-applied ytkit-embed-active class');
                    }
                ...
                this._player = video;
                this._isActive = true;
                DebugManager.log('Embed', 'Player created successfully with persistence');
                return video;
            },
            ...
            _setupSponsorSkip(video, segments) {
                    ...
                    for (const seg of segments) {
                        if (currentTime >= seg.start && currentTime < seg.end) {
                            ...
                            if (appState.settings.skipSponsors) {
                                video.currentTime = seg.end + 0.1;
                                DebugManager.log('Embed', `Skipped ${seg.category}: ${seg.start}s - ${seg.end}s`);
                            }
                            return;
                        }
                    }
                ...
                DebugManager.log('Embed', `SponsorBlock: ${segments.length} segments loaded`);
            },
            ...
            async activateEmbed(showAlerts = false) {
                ...
                if (!serverOk) {
                    DebugManager.log('Embed', 'Server not running');
                    if (showAlerts) {
                ...
                if (!streamData || !streamData.success) {
                    DebugManager.log('Embed', 'Failed to get stream URLs');
                    if (showAlerts) {
        ...
        },
        {
            ...
            async _tryEmbed() {
                ...
                DebugManager.log('Embed', 'Auto-embed triggered for:', videoId);
                ...
                if (embedFeature && typeof embedFeature.activateEmbed === 'function') {
                    ...
                    DebugManager.log('Embed', 'Auto-embed result:', success ? 'success' : 'failed');
                }
        ...
        // ─── Comment Navigator ───
        {
            ...
            description: 'Quick-jump between top-level comments with floating up/down buttons',
            group: 'Clutter',
            icon: 'message-square',
            _container: null,
            _currentIndex: -1,
            init() {
                ...
                const createNav = () => {
                    ...
                    const makeBtn = (label, svg, direction) => {
                        const btn = document.createElement('button');
                        btn.title = label;
                        TrustedHTML.setHTML(btn, svg);
                        btn.style.cssText = 'width:40px;height:40px;border-radius:50%;background:rgba(0,0,0,0.8);border:1px solid rgba(255,255,255,0.15);color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.2s;backdrop-filter:blur(8px);';
                    ...
                    nav.appendChild(makeBtn('Previous comment', '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 15l-6-6-6 6"/></svg>', -1));
                    nav.appendChild(makeBtn('Next comment', '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>', 1));
                    document.body.appendChild(nav);
                    self._container = nav;
                };
                addNavigateRule('commentNav', () => { self._currentIndex = -1; self._container?.remove(); self._container = null; setTimeout(createNav, 2000); });
            },
            _navigate(direction) {
    ...
    async function buildStatisticsDashboard() {
        ...
        container.querySelector('.ytkit-stats-reset').onclick = async () => {
            const backup = JSON.parse(JSON.stringify(await StatsTracker.getAll?.() || {}));
            await StatsTracker.reset();
            const parent = container.parentElement;
            container.remove();
            parent?.appendChild(await buildStatisticsDashboard());
            showToast('Statistics reset', '#f97316', { duration: 5, action: { text: 'Undo', onClick: async () => {
                if (backup && Object.keys(backup).length) { for (const [k, v] of Object.entries(backup)) await GM_setValue(k, v); }
                const p2 = parent; if (p2) { p2.querySelector('.ytkit-stats-container')?.remove(); p2.appendChild(await buildStatisticsDashboard()); }
                showToast('Statistics restored', '#22c55e');
            }}});
        };
    ...
    async function buildProfilesUI() {
        ...
        container.querySelectorAll('.ytkit-profile-apply').forEach(btn => {
            btn.onclick = async (e) => {
                ...
                showToast(`Profile "${profileKey}" applied — reloading...`, '#22c55e');
        ...
        container.querySelectorAll('.ytkit-profile-delete').forEach(btn => {
            btn.onclick = async (e) => {
                ...
                const backup = await ProfilesManager.getCustomProfile?.(profileKey) || null;
                await ProfilesManager.deleteCustomProfile(profileKey);
                const parent = container.parentElement;
                container.remove();
                parent?.appendChild(await buildProfilesUI());
                showToast(`Profile "${profileKey}" deleted`, '#f97316', { duration: 5, action: { text: 'Undo', onClick: async () => {
                    if (backup) await ProfilesManager.saveCustomProfile?.(profileKey, backup);
                    const p2 = parent; if (p2) { p2.querySelector('.ytkit-profiles-container')?.remove(); p2.appendChild(await buildProfilesUI()); }
                    showToast(`Profile "${profileKey}" restored`, '#22c55e');
                }}});
            };
        });
...
`;
        ...
        if (lastSeenVer !== CURRENT_VER) {
            ...
            versionSpan.onclick = () => {
                ...
                badge.remove();
                showToast('v19: Auto-Resume, Speed OSD, Watch Time, Comment Nav, Speed Badge, Theater Scroll, Screenshot Format + 15 bug fixes & improvements', '#3b82f6', { duration: 6 });
            };
        }
...
@keyframes slideDown {
    ...
    async function main() {
        ...
        features.forEach(f => {
            // For array-toggle sub-features, skip init (parent handles the work)
        ...
        console.log('[YTKit] v19 Initialized');
        DebugManager.log('Init', 'YTKit v19 started', { page: appState.currentPage, features: Object.keys(appState.settings).filter(k => appState.settings[k]).length });
    }
