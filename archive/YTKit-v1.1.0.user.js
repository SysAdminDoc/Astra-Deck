...
// @version      1.1.0
// @description  Ultimate YouTube customization with ad blocking, VLC streaming, video/channel hiding, playback enhancements, sticky video, ChapterForge AI chapters, and more. Uses YTYT-Downloader for local downloads.
...
(function() {
    ...
    const features = [
        ...
        },
        {
            ...
            init() {
                ...
                this._styleEl = GM_addStyle(`
                    ...
                    .ytkit-po-logo yt-icon, .ytkit-po-logo yt-icon span, .ytkit-po-logo yt-icon span div {
                        padding: 0; margin: 0;
                    }
                    .ytkit-po-logo ytd-logo { margin-right: -12px; }
                    .ytkit-po-gear {
                        padding: 0;
                        margin: 0 0 0 -7px;
                    }
        ...
        // ═══════════════════════════════════════════════════════════════════
        {
            id: 'chapterForge',
            name: 'ChapterForge',
            description: 'AI-powered chapter & POI generation for YouTube videos using WebLLM + Whisper transcription',
            group: 'ChapterForge',
            icon: 'player',
            isParent: true,
            ...
            _CF_PROVIDERS: {
                builtin:    { name: 'Built-in (no setup)', endpoint: null, needsKey: false, defaultModel: null },
                openai:     { name: 'OpenAI', endpoint: 'https://api.openai.com/v1/chat/completions', needsKey: true, defaultModel: 'gpt-4o-mini' },
                ollama:     { name: 'Ollama (local)', endpoint: 'http://localhost:11434/v1/chat/completions', needsKey: false, defaultModel: 'llama3.2' },
                openrouter: { name: 'OpenRouter', endpoint: 'https://openrouter.ai/api/v1/chat/completions', needsKey: true, defaultModel: 'meta-llama/llama-3.2-3b-instruct:free' },
                custom:     { name: 'Custom Endpoint', endpoint: '', needsKey: false, defaultModel: '' },
            },

            _CF_CACHE_PREFIX: 'cf_cache_',
            _CF_COLORS: ['#7c3aed', '#2563eb', '#059669', '#d97706', '#dc2626', '#7c3aed'],
            ...
            _gmPostJson(url, data, extraHeaders = {}) {
                return new Promise((resolve, reject) => {
            ...
            // ═══════════════════════════════════════════
            //  TRANSCRIPT FETCHER — Uses YTKit's TranscriptService as primary,
            //  then ChapterForge-specific GM fallbacks, then Innertube get_transcript
            // ═══════════════════════════════════════════
            async _fetchTranscript(videoId, onStatus) {
                ...
                if (transcriptMethod === 'whisper-only') {
                    ...
                    return null; // caller will fall through to Whisper
                }

                this._log('=== Fetching transcript for:', videoId, '(method:', transcriptMethod, ') ===');
                onStatus?.('Fetching transcript...', 'loading', 5);

                // ── PRIMARY: Use YTKit's TranscriptService (proven 5-method failover) ──
                try {
                    ...
                    if (trackData?.tracks?.length) {
                        ...
                        if (selectedTrack.baseUrl) {
                            // Try fetching content via TranscriptService's parser
                            try {
                                ...
                                if (tsSegments?.length) {
                                    ...
                                    // Adapt TranscriptService format (startMs/endMs) to ChapterForge format (start/dur/text)
                            ...
                            // Fallback: GM-backed caption download with auth headers
            ...
            async _gmDownloadCaptions(trackOrFirst, videoId, allTracks) {
                // Select best track from allTracks if provided
                ...
                // Method A: GM_xmlhttpRequest with auth headers
                const authHeaders = await this._buildSapisidAuth() || {};
            ...
            async _scrapeTranscriptFromDOM() {
                // Check for existing transcript segments
            ...
            _buildTranscriptText(segments, maxChars = 12000) {
                ...
                for (const seg of segments) { if (seg.start - lastBlockStart >= 30 || blocks.length === 0) { if (currentBlock.texts.length) blocks.push(currentBlock); currentBlock = { start: seg.start, texts: [] }; lastBlockStart = seg.start; } currentBlock.texts.push(seg.text); }
                if (currentBlock.texts.length) blocks.push(currentBlock);
                let result = '';
                for (const b of blocks) { const line = `[${this._formatTime(b.start)}] ${b.texts.join(' ')}\n`; if (result.length + line.length > maxChars) break; result += line; }
                return result;
            },
            ...
            async _downloadTranscript() {
                ...
                if (!videoId) return showToast('No video detected', '#ef4444');
                ...
                this._updateStatus(`Downloaded ${segments.length} segments`, 'ready', 100);
                showToast(`Transcript downloaded (${segments.length} segments)`, '#22c55e');
                if (btn) { btn.disabled = false; btn.textContent = 'Download Transcript'; btn.classList.remove('cf-loading'); }
            },
            ...
            async _startLiveTracking() {
                ...
                showToast('Live chapter tracking started', '#7c3aed');

                // Poll for new transcript segments every 30s
                this._liveIntervalId = setInterval(async () => {
                    const videoId = this._getVideoId();

                    // Failsafe: stop if video ended, no longer live, or navigated away
                    if (!videoId || this._isVideoEnded()) {
                        this._log('Live: video ended or no video, stopping');
                        this._stopLiveTracking();
                        showToast('Live tracking stopped — video ended', '#7c3aed');
                        return;
                    }
                    if (!this._isLiveVideo()) {
                        ...
                        if (this._liveNoNewCount >= 3) {
                            this._stopLiveTracking();
                            showToast('Live tracking stopped — stream ended', '#7c3aed');
                            return;
                        }
                    }
                    ...
                    try {
                        ...
                        if (segments?.length) {
                            // Merge new segments (by start time dedup)
                            ...
                            if (added > 0) {
                                ...
                                this._liveNoNewCount++;
                                this._log('Live: no new segments, stale count:', this._liveNoNewCount);
                            }

                            // Failsafe: 5 consecutive polls with no new content = stop (2.5 min)
                            if (this._liveNoNewCount >= 5) {
                                this._log('Live: no new content for 5 polls, stopping');
                                this._stopLiveTracking();
                                showToast('Live tracking stopped — no new content detected', '#f59e0b');
                                return;
                            }

                            // Regenerate chapters every 2 minutes if enough content
                            const now = Date.now();
                            if (this._liveAccumulated.length >= 10 && now - this._liveLastGenTime > 120000) {
                                ...
                                if (provider === 'builtin') {
                                    ...
                                    try {
                                        const transcriptText = this._buildTranscriptText(this._liveAccumulated);
                                        const durationMin = Math.ceil(duration / 60);
                                        const systemPrompt = `You are a video chapter generator for a LIVE stream. Given a timestamped transcript, output ONLY a JSON object with this structure:\n{"chapters":[{"start":0,"title":"Introduction"}],"pois":[{"time":45,"label":"Key moment"}]}\nRules: start in seconds, first chapter at 0, 3-8 chapters, 2-6 POIs, max 5 word titles. Video is ~${durationMin} min so far. Output ONLY JSON.`;
                                        const rawText = await this._callLlmApi(systemPrompt, `Generate chapters for this live stream transcript:\n\n${transcriptText}`, null);
                            ...
                            if (this._liveNoNewCount >= 5) {
                                this._stopLiveTracking();
                                showToast('Live tracking stopped — no transcript available', '#f59e0b');
                                return;
                            }
                        }
                    } catch(e) {
                        ...
                        if (this._liveNoNewCount >= 5) {
                            this._stopLiveTracking();
                            showToast('Live tracking stopped — repeated errors', '#ef4444');
                        }
                    }
                }, 30000);
            },
            ...
            async _batchProcessSubscriptions() {
                ...
                // Collect all video links on the subscriptions page
                ...
                // Also check thumbnail links
                document.querySelectorAll('a#thumbnail[href*="watch"]').forEach(a => {
                    const match = a.href?.match(/[?&]v=([a-zA-Z0-9_-]{11})/);
                    if (match) videoLinks.add(match[1]);
                });

                if (!videoLinks.size) { showToast('No videos found on this page', '#ef4444'); return; }

                // Filter out already-cached
                const uncached = [...videoLinks].filter(id => !this._getCachedData(id));
                if (!uncached.length) { showToast(`All ${videoLinks.size} videos already processed`, '#22c55e'); return; }

                this._batchProcessing = true;
                this._batchProgress = { done: 0, total: uncached.length, current: '' };
                showToast(`Processing ${uncached.length} videos...`, '#7c3aed');
                this._updateBatchUI();
                ...
                for (const videoId of uncached) {
                    ...
                    try {
                        // Fetch transcript for this video (uses GM so no navigation needed)
                        const segments = await this._fetchTranscript(videoId, null);
                        if (segments?.length) {
                            const provider = appState.settings.cfLlmProvider || 'builtin';
                            let data;
                            if (provider === 'builtin') {
                                ...
                                try {
                                    ...
                                    const systemPrompt = `You are a video chapter generator. Output ONLY a JSON object: {"chapters":[{"start":0,"title":"..."}],"pois":[{"time":0,"label":"..."}]}. start in seconds, first at 0, 3-8 chapters, 2-6 POIs, max 5 word titles. Output ONLY JSON.`;
                ...
                this._removeBatchUI();
                showToast(`Batch complete: ${done}/${uncached.length} videos processed`, '#22c55e');
            },
            ...
            _injectSubscriptionsButton() {
                ...
                // Place alongside existing YTKit buttons in the masthead header
                const headerButtons = document.querySelector('#masthead #end #buttons');
                ...
                // Insert near existing YTKit buttons (after Hide All, before VLC)
            ...
            async _loadWhisperPipeline(onProgress) {
                ...
                // YouTube's CSP blocks import() from external CDNs.
                // Try blob-URL approach: fetch lib via GM (bypasses CSP), then import from blob
                let transformers;
                const cdnUrl = 'https://cdn.jsdelivr.net/npm/@huggingface/transformers@3.4.0/dist/transformers.min.js';
                try {
                    this._log('Whisper: trying GM fetch + blob import...');
                    ...
                    this._log('Whisper: blob import succeeded');
                } catch(e1) {
                    this._log('Whisper: blob import failed:', e1.message, '- trying direct import...');
                    try { transformers = await import(cdnUrl); }
                    catch(e2) {
                        this._warn('Whisper: all import methods blocked by CSP. Switch to "Captions Only" in ChapterForge settings.');
                        throw new Error('Whisper blocked by YouTube CSP. Use "Captions Only" transcript mode, or install via @require.');
                    }
                }
            ...
            async _whisperTranscribe(videoId, onStatus) {
                ...
                if (!methods.length) methods.push('innertube', 'cobalt'); // safety fallback
            ...
            async _callLlmApi(systemPrompt, userPrompt, onStatus) {
                ...
                this._log('LLM API call:', endpoint, 'model:', model);
                onStatus?.('Calling LLM API...', 'loading', 80);

                const headers = { 'Content-Type': 'application/json' };
                if (apiKey) headers['Authorization'] = `Bearer ${apiKey}`;
                // OpenRouter needs extra headers
                if ((appState.settings.cfLlmProvider || '') === 'openrouter') {
                ...
                const data = await this._gmPostJson(endpoint, body, headers);
            ...
            // ═══ BUILT-IN HEURISTIC CHAPTER GENERATOR (zero dependencies) ═══
            _generateChaptersHeuristic(segments, duration) {
                this._log('Using built-in heuristic generator on', segments.length, 'segments');
                const totalSecs = duration || segments[segments.length - 1]?.start + 30 || 300;

                // Build time-bucketed text blocks (30-second windows)
                const bucketSize = 30;
                const buckets = [];
                for (const seg of segments) {
                    const idx = Math.floor(seg.start / bucketSize);
                    while (buckets.length <= idx) buckets.push({ start: buckets.length * bucketSize, words: [], text: '' });
                    const words = seg.text.toLowerCase().replace(/[^\w\s'-]/g, '').split(/\s+/).filter(w => w.length > 2);
                    buckets[idx].words.push(...words);
                    buckets[idx].text += ' ' + seg.text;
                }

                // Stopwords
                const stops = new Set(['the','and','that','this','with','for','are','was','were','been','have','has','had','not','but','what','all','can','her','his','from','they','will','one','its','also','just','more','about','would','there','their','which','could','other','than','then','these','some','them','into','only','your','when','very','most','over','such','after','know','like','going','right','think','really','want','well','here','look','make','come','how','did','get','got','say','said','because','way','still','being','those','where','back','does','take','much','many','being','through','before']);
                const meaningful = (w) => !stops.has(w) && w.length > 3;

                // Compute vocabulary shift between adjacent bucket groups
                const groupSize = Math.max(2, Math.floor(buckets.length / 12)); // ~12 groups
                const groups = [];
                for (let i = 0; i < buckets.length; i += groupSize) {
                    ...
                    for (const b of slice) {
                        allText.push(b.text.trim());
                        for (const w of b.words) { if (meaningful(w)) wordFreq[w] = (wordFreq[w] || 0) + 1; }
                    }
                    const topWords = Object.entries(wordFreq).sort((a, b) => b[1] - a[1]).slice(0, 8).map(e => e[0]);
                    groups.push({ start: slice[0]?.start || 0, topWords, text: allText.join(' ').trim(), wordSet: new Set(Object.keys(wordFreq)) });
                }

                // Find topic boundaries by vocabulary divergence
                const boundaries = [{ start: 0, groupIdx: 0 }];
                for (let i = 1; i < groups.length; i++) {
                    const prev = groups[i - 1].wordSet;
                    const curr = groups[i].wordSet;
                    ...
                    if (jaccard < 0.25 || (jaccard < 0.4 && i > boundaries[boundaries.length - 1].groupIdx + 1)) {
                        boundaries.push({ start: groups[i].start, groupIdx: i });
                    }
                }

                // Limit to 3-8 chapters
                while (boundaries.length > 8) {
                    // Remove the boundary with highest similarity to its predecessor
                    let minDist = Infinity, minIdx = 1;
                    for (let i = 1; i < boundaries.length; i++) {
                        const dist = boundaries[i].start - boundaries[i - 1].start;
                        if (dist < minDist) { minDist = dist; minIdx = i; }
                    }
                    boundaries.splice(minIdx, 1);
                }
                if (boundaries.length < 3 && groups.length >= 3) {
                    // Add evenly spaced chapters
                    const step = Math.floor(groups.length / 4);
                    for (let i = step; i < groups.length && boundaries.length < 5; i += step) {
                        if (!boundaries.find(b => Math.abs(b.start - groups[i].start) < bucketSize * 2)) {
                            boundaries.push({ start: groups[i].start, groupIdx: i });
                        }
                    }
                ...
                // Generate chapter titles from top keywords and context
                const chapters = boundaries.map((b, i) => {
                    const endIdx = i < boundaries.length - 1 ? boundaries[i + 1].groupIdx : groups.length;
                    const relevantGroups = groups.slice(b.groupIdx, endIdx);
                    const allTopWords = relevantGroups.flatMap(g => g.topWords);
                    const freq = {};
                    for (const w of allTopWords) freq[w] = (freq[w] || 0) + 1;
                    const ranked = Object.entries(freq).sort((a, b) => b[1] - a[1]).slice(0, 4).map(e => e[0]);

                    let title;
                    if (i === 0) {
                        title = 'Introduction';
                    } else if (ranked.length >= 2) {
                        // Build natural title: "Topic on Subject" or "Discussing Topic"
                        const cap = w => w.charAt(0).toUpperCase() + w.slice(1);
                        if (ranked.length >= 3) {
                            title = `${cap(ranked[0])}, ${cap(ranked[1])} ${cap(ranked[2])}`;
                        } else {
                            title = `${cap(ranked[0])} ${cap(ranked[1])}`;
                        }
                    } else if (ranked.length === 1) {
                        title = ranked[0].charAt(0).toUpperCase() + ranked[0].slice(1);
                    } else {
                        title = `Section ${i + 1}`;
                    }
                    if (!title || title.length < 3) title = `Section ${i + 1}`;
                    return { start: Math.round(b.start), title: title.slice(0, 50) };
                });
                ...
                // Generate POIs: find segments with question marks, emphasis words, or topic introductions
                const poiCandidates = [];
                const emphasisPatterns = [/important|key point|remember|note that|crucial|breaking|announce|reveal|spoiler|surprise|actually|amazing|incredible|crazy/i];
                for (const seg of segments) {
                    let score = 0;
                    if (seg.text.includes('?')) score += 2;
                    for (const pat of emphasisPatterns) { if (pat.test(seg.text)) score += 3; }
                    if (seg.text.length > 80) score += 1; // Longer = more substantive
                    if (score >= 2) poiCandidates.push({ time: Math.round(seg.start), label: seg.text.slice(0, 70).trim(), score });
                }
                poiCandidates.sort((a, b) => b.score - a.score);
                // Keep top 2-5, spaced at least 60s apart
                const pois = [];
                for (const p of poiCandidates) {
                    if (pois.length >= 5) break;
                    if (pois.some(existing => Math.abs(existing.time - p.time) < 60)) continue;
                    pois.push(p);
                }
                pois.sort((a, b) => a.time - b.time);

                this._log('Heuristic result:', chapters.length, 'chapters,', pois.length, 'POIs');
            ...
            async _generateChapters(videoId, onStatus) {
                ...
                try {
                    ...
                    // Try captions first (unless whisper-only)
                    if (transcriptMethod !== 'whisper-only') {
                        segments = await this._fetchTranscript(videoId, onStatus);
                    }

                    // Whisper fallback (unless captions-only)
                    ...
                    let data;
                    if (provider === 'builtin') {
                        // ── Built-in heuristic (no API, no ML) ──
                        onStatus?.('Analyzing transcript (built-in)...', 'loading', 60);
                        data = this._generateChaptersHeuristic(segments, duration);
                    } else {
                        // ── API-based generation (OpenAI-compatible) ──
                        const endpoint = this._getLlmEndpoint();
                        if (!endpoint) { onStatus?.('No LLM endpoint configured — check ChapterForge settings', 'error', 0); this._isGenerating = false; return null; }
                        const needsKey = this._CF_PROVIDERS[provider]?.needsKey;
                        if (needsKey && !appState.settings.cfLlmApiKey) { onStatus?.(`API key required for ${this._CF_PROVIDERS[provider]?.name || provider}`, 'error', 0); this._isGenerating = false; return null; }

                        onStatus?.('Preparing transcript for AI...', 'loading', 55);
                        const transcriptText = this._buildTranscriptText(segments);
                        const durationMin = Math.ceil(duration / 60);

                        const systemPrompt = `You are a video chapter generator. Given a timestamped transcript, output ONLY a JSON object (no markdown, no backticks) with this exact structure:\n{"chapters":[{"start":0,"title":"Introduction"},{"start":120,"title":"Topic Name"}],"pois":[{"time":45,"label":"Key moment description"}]}\n\nRules:\n- "start" values are in seconds (integers)\n- First chapter MUST start at 0\n- Create 3-8 chapters based on topic changes\n- Create 2-6 POIs marking the most valuable/interesting moments\n- Chapter titles: max 5 words, descriptive\n- POI labels: max 8 words, specific\n- Video is ${durationMin} minutes long\n- Output ONLY the JSON object, nothing else`;
                        const userPrompt = `Generate chapters and points of interest for this video transcript:\n\n${transcriptText}`;
                        ...
                        if (!data?.chapters?.length) {
                            ...
                            onStatus?.('Failed to parse AI response — try a different model or use Built-in mode', 'error', 0);
                            this._isGenerating = false; return null;
                        }
                    }
            ...
            _renderProgressBarOverlay() {
                document.querySelectorAll('.cf-bar-overlay').forEach(el => el.remove());
                ...
                if (s.cfShowChapters && this._chapterData.chapters.length) {
                    this._chapterData.chapters.forEach((ch, i) => {
                        const left = (ch.start / duration) * 100; const width = ((ch.end - ch.start) / duration) * 100;
                        const color = this._CF_COLORS[i % this._CF_COLORS.length];
                        const seg = document.createElement('div'); seg.className = 'cf-bar-chapter';
                        seg.style.cssText = `left:${left}%;width:${width}%;background:${color};--cf-chapter-opacity:${s.cfChapterOpacity || 0.35}`;
                        seg.addEventListener('click', (e) => { e.stopPropagation(); this._seekTo(ch.start); });
                        const tip = document.createElement('div'); tip.className = 'cf-bar-tooltip cf-chapter-tip';
                ...
                if (s.cfShowPOIs && this._chapterData.pois.length) {
                    this._chapterData.pois.forEach(p => {
                        ...
                        const marker = document.createElement('div'); marker.className = 'cf-bar-poi';
                        marker.style.cssText = `left:${left}%;background:${poiColor}`;
                        marker.addEventListener('click', (e) => { e.stopPropagation(); this._seekTo(p.time); });
                        const tip = document.createElement('div'); tip.className = 'cf-bar-tooltip cf-poi-tip';
                ...
                // Transcript hover layer — shows transcript text at cursor position on the bar
                if (this._lastTranscriptSegments?.length) {
                    ...
                    overlay.addEventListener('mousemove', (e) => {
                        ...
                        for (const seg of this._lastTranscriptSegments) {
                            if (seg.start <= hoverTime && hoverTime <= seg.start + (seg.dur || 5)) { best = seg; break; }
                            if (seg.start > hoverTime) break;
                            best = seg;
                        }
                        if (best) {
                            const text = best.text.length > 90 ? best.text.slice(0, 87) + '...' : best.text;
                            transcriptTip.textContent = text;
                            transcriptTip.style.opacity = '1';
                            transcriptTip.style.left = Math.max(5, Math.min(rect.width - 200, e.clientX - rect.left)) + 'px';
                        } else {
            ...
            _renderPanel() {
                ...
                const hasData = !!this._chapterData; const s = appState.settings;
                let chaptersHTML = '', settingsHTML = '';
                if (this._activeTab === 'chapters') {
                    if (hasData && this._chapterData.chapters.length) {
                        chaptersHTML = `<div class="cf-section-label">Chapters</div><ul class="cf-chapter-list">`;
                        this._chapterData.chapters.forEach((c, i) => { const color = this._CF_COLORS[i % this._CF_COLORS.length]; chaptersHTML += `<li class="cf-chapter-item" data-cf-seek="${c.start}"><span class="cf-chapter-dot" style="background:${color}"></span><span class="cf-chapter-time">${this._formatTime(c.start)}</span><span class="cf-chapter-title">${c.title}</span></li>`; });
                        chaptersHTML += `</ul>`;
                        if (this._chapterData.pois.length) {
                            const poiColor = s.cfPoiColor || '#ff6b6b';
                            chaptersHTML += `<div class="cf-section-label">Points of Interest</div><ul class="cf-chapter-list">`;
                            this._chapterData.pois.forEach(p => { chaptersHTML += `<li class="cf-chapter-item" data-cf-seek="${p.time}"><span class="cf-chapter-dot" style="background:${poiColor}"></span><span class="cf-chapter-time">${this._formatTime(p.time)}</span><span class="cf-chapter-title">${p.label}<span class="cf-poi-badge">POI</span></span></li>`; });
                            chaptersHTML += `</ul>`;
                        }
                    } else {
                        chaptersHTML = `<div class="cf-empty"><svg viewBox="0 0 24 24" style="width:40px;height:40px;fill:rgba(255,255,255,0.08);margin-bottom:12px"><path d="M3 13h2v-2H3v2zm0 4h2v-2H3v2zm0-8h2V7H3v2zm4 4h14v-2H7v2zm0 4h14v-2H7v2zM7 7v2h14V7H7z"/></svg><div>No chapters generated yet</div><div style="margin-top:4px;font-size:11px;color:rgba(255,255,255,0.15)">Click Generate to analyze this video</div></div>`;
                    }
                    ...
                    const showApiFields = s.cfLlmProvider && s.cfLlmProvider !== 'builtin';
                    ...
                    settingsHTML = `
                        ...
                        ` : '<div style="font-size:10px;color:rgba(255,255,255,0.15);margin:-2px 0 8px;padding-left:2px">Analyzes transcript patterns locally. No API key needed.</div>'}
                        ...
                        <div class="cf-settings-row"><span class="cf-settings-label">Cached Videos</span><span style="font-size:12px;color:rgba(255,255,255,0.4)">${this._countCache()}</span></div>
                        <button class="cf-clear-btn" id="cf-clear-cache">Clear All Cache</button>
                    `;
                }

                TrustedHTML.setHTML(this._panelEl, `
                    <div class="cf-panel-header"><div><span class="cf-panel-title">ChapterForge</span><span class="cf-panel-version">v21.5</span></div><button class="cf-panel-close" id="cf-close">&times;</button></div>
                    <div class="cf-tab-bar"><div class="cf-tab ${this._activeTab === 'chapters' ? 'active' : ''}" data-cf-tab="chapters">Chapters</div><div class="cf-tab ${this._activeTab === 'settings' ? 'active' : ''}" data-cf-tab="settings">Settings</div></div>
                    <div class="cf-panel-body">
                        ...
                        ${this._activeTab === 'chapters' ? chaptersHTML : settingsHTML}
                    </div>
                `);
                ...
                this._panelEl.querySelector('#cf-live-track')?.addEventListener('click', () => { if (self._liveIntervalId) { self._stopLiveTracking(); showToast('Live tracking stopped', '#7c3aed'); } else { self._startLiveTracking(); } self._renderPanel(); });
                this._panelEl.querySelectorAll('.cf-tab').forEach(tab => { tab.addEventListener('click', () => { self._activeTab = tab.dataset.cfTab; self._renderPanel(); }); });
                ...
                // Provider select — when changed, auto-set model to default for that provider
                this._panelEl.querySelector('#cf-provider-select')?.addEventListener('change', (e) => {
                    ...
                    if (info?.defaultModel && !appState.settings.cfLlmModel?.trim()) appState.settings.cfLlmModel = info.defaultModel;
                    if (prov !== 'custom' && info?.defaultModel) appState.settings.cfLlmModel = info.defaultModel;
                ...
                this._panelEl.querySelector('#cf-clear-cache')?.addEventListener('click', () => { self._clearCache(); self._chapterData = null; self._renderPanel(); self._renderProgressBarOverlay(); showToast('ChapterForge cache cleared', '#7c3aed'); });
            },

            _updateStatus(text, state, pct) {
                // Mini progress indicator next to the player button
            ...
            async _handleGenerate() {
                ...
                if (data) { this._chapterData = data; this._currentDuration = this._getVideoDuration(); this._renderPanel(); this._renderProgressBarOverlay(); showToast(`${data.chapters.length} chapters generated`, '#7c3aed'); }
            ...
            _onVideoChange() {
                ...
                this._waitForPlayer().then(() => {
                    ...
                    // Inject player button in manual mode, or if setting enabled
                    ...
                    // Auto mode: generate if no cache and within duration limit
                    ...
                    // Live video: auto-start tracking in auto mode
            ...
            init() {
                const css = `
                    ...
                    .cf-bar-overlay { position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:35; }
                    .cf-bar-chapter { position:absolute;top:0;height:100%;opacity:var(--cf-chapter-opacity,0.35);transition:opacity 0.15s;pointer-events:auto;cursor:pointer; } .cf-bar-chapter:hover { opacity:0.6; }
                    .cf-bar-poi { position:absolute;top:50%;width:10px;height:10px;transform:translate(-50%,-50%) rotate(45deg);border-radius:2px;pointer-events:auto;cursor:pointer;z-index:36;transition:all 0.2s;box-shadow:0 0 6px rgba(255,107,107,0.4); } .cf-bar-poi:hover { transform:translate(-50%,-50%) rotate(45deg) scale(1.6);box-shadow:0 0 12px rgba(255,107,107,0.7),0 0 24px rgba(255,107,107,0.3); }

                    .cf-bar-tooltip { position:absolute;bottom:22px;left:50%;transform:translateX(-50%);padding:6px 12px;border-radius:8px;font-size:11px;white-space:nowrap;pointer-events:none;z-index:45;opacity:0;transition:opacity 0.15s; }

                    .cf-chapter-tip { background:rgba(15,15,20,0.92);color:#e0e0e8;border:1px solid rgba(124,58,237,0.25);box-shadow:0 4px 16px rgba(0,0,0,0.5);display:flex;gap:8px;align-items:center; }
                    ...
                    .cf-poi-tip { background:linear-gradient(135deg,rgba(30,10,10,0.95),rgba(15,15,20,0.95));color:#fca5a5;border:1px solid rgba(255,107,107,0.35);box-shadow:0 4px 20px rgba(255,107,107,0.15),0 0 40px rgba(255,107,107,0.05);display:flex;gap:6px;align-items:center;animation:cfGlow 2s ease-in-out infinite; }
                    ...
                    .cf-bar-poi .cf-bar-tooltip { transform:translateX(-50%) rotate(-45deg);bottom:16px; }

                    .cf-transcript-tip { position:absolute;bottom:28px;background:rgba(10,10,15,0.92);color:rgba(255,255,255,0.7);padding:5px 10px;border-radius:6px;font-size:10px;max-width:280px;white-space:normal;word-wrap:break-word;pointer-events:none;z-index:34;opacity:0;transition:opacity 0.12s;border:1px solid rgba(255,255,255,0.06);box-shadow:0 2px 8px rgba(0,0,0,0.4);line-height:1.4; }

                    .cf-btn-active { animation:cfBtnPulse 1.5s infinite; }
                    @keyframes cfBtnPulse { 0%,100%{opacity:1} 50%{opacity:0.5} }
                `;
                this._styleElement = injectStyle(css, 'chapterForge', true);

                this._navHandler = () => {
                    this._onVideoChange();
                    // Inject subscriptions batch button on feed page
                    if (window.location.pathname.startsWith('/feed/subscriptions')) {
                        setTimeout(() => this._injectSubscriptionsButton(), 1000);
                    }
                    // Stop live tracking when navigating away from video
                ...
                this._clickHandler = (e) => {
                    ...
                    // Use bounding rect check — DOM containment fails after _renderPanel()
                    // replaces inner content via TrustedHTML.setHTML(), orphaning the clicked node
                ...
                console.log('[ChapterForge] Integrated into YTKit — Provider:', appState.settings.cfLlmProvider || 'builtin');
            },
        ...
        {
            id: 'cfLlmProvider', name: 'Chapter AI Provider', description: 'Built-in (local heuristic, no setup), or connect to OpenAI / Ollama / OpenRouter / custom endpoint',
            group: 'ChapterForge', icon: 'settings-2', type: 'select', dependsOn: 'chapterForge',
            options: { 'builtin': 'Built-in (no setup)', 'openai': 'OpenAI', 'ollama': 'Ollama (local)', 'openrouter': 'OpenRouter', 'custom': 'Custom Endpoint' },
            init() {}, destroy() {}
        },
        {
    ...
    function buildSettingsPanel() {
        ...
        const categoryOrder = ['Interface', 'Appearance', 'Content', 'Video Hider', 'Video Player', 'Playback', 'Ad Blocker', 'SponsorBlock', 'Quality', 'Clutter', 'Live Chat', 'Action Buttons', 'Player Controls', 'Downloads', 'ChapterForge', 'Advanced'];
