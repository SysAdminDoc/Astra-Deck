...
(function ytAdBlockBootstrap() {
    ...
    function pageContextEngine(cfg, W) {
        ...
        DebugManager.log('AdBlock', 'Proxy engine injected into page context');
    }
    ...
    DebugManager.log('AdBlock', 'Bootstrap active — proxies in page context, CSS/DOM/SSAP in sandbox');
})();
...
- Your output must be a neutral index. Nothing more.`;
                ...
                const effectivePrompt = appState.settings.cfCustomSummaryPrompt || (summaryMode === 'timestamped' ? timestampedPrompt : paragraphPrompt);
                ...
                if (provider === 'builtin') {
                    ...
                    try {
                        ...
                        const userMsg = summaryMode === 'timestamped'
                            ...
                            : `Summarize this ${durationMin}-minute video${vidTitle ? ' titled "' + vidTitle + '"' : ''} in a few clear sentences.\n\n${transcriptText}`;
                        this._log('Summary: calling LLM API, prompt:', transcriptText.length, 'chars');
...
Rules: start in seconds (integers), first at 0, 3-8 chapters, 2-6 POIs. Titles 3-7 words, specific to content discussed. Output ONLY JSON.`;
            ...
            _renderPanel() {
                ...
                TrustedHTML.setHTML(this._panelEl, `
                    <div class="cf-panel-header"><div><span class="cf-panel-title">ChapterForge</span><span class="cf-panel-version">v24.9</span></div><button class="cf-panel-close" id="cf-close">&times;</button></div>
                    <div class="cf-tab-bar"><div class="cf-tab ${this._activeTab === 'chapters' ? 'active' : ''}" data-cf-tab="chapters">Chapters</div><div class="cf-tab ${this._activeTab === 'tools' ? 'active' : ''}" data-cf-tab="tools">Tools</div><div class="cf-tab ${this._activeTab === 'analysis' ? 'active' : ''}" data-cf-tab="analysis">Analysis</div><div class="cf-tab ${this._activeTab === 'notes' ? 'active' : ''}" data-cf-tab="notes">Notes</div><div class="cf-tab ${this._activeTab === 'settings' ? 'active' : ''}" data-cf-tab="settings">Settings</div></div>
            ...
            init() {
                ...
                if (appState.settings?.cfDebugLog) console.log('[ChapterForge] v24.9 initialized — Provider:', appState.settings.cfLlmProvider || 'builtin');
            },
...
`;
        ...
        if (lastSeenVer !== CURRENT_VER) {
            ...
            versionSpan.onclick = () => {
                ...
                badge.remove();
                showToast('v24.9: Qwen3-32B default, streaming fix', '#3b82f6', { duration: 6 });
            };
        }
...
@keyframes slideDown {
    ...
    function main() {
        ...
        console.log(`%c[YTKit] v24.9 Initialized${isSafeMode ? ' (SAFE MODE)' : ''}`, 'color: #3b82f6; font-weight: bold; font-size: 14px;');
    }
