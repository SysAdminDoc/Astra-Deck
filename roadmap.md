...
| Release | Headline | Key features |
|---------|----------|--------------|
| v3.7.0 | Transcript export, privacy hardening | Transcript export (Copy/.txt/.srt/LLM), expanded URL tracking-param strip (UTM + click IDs), `credentials: 'omit'` default in EXT_FETCH proxy |
| v3.8.0 | Toolbar popup, digital wellbeing, profiles | Toolbar popup w/ 15 quick-toggles + live filter, `settingsProfiles` save/load/import/export, `digitalWellbeing` break reminders + daily cap, `videoRotation`, `frameByFrameButtons`, `YTKIT_SETTING_CHANGED` live toggles |
| v3.9.0 | Visual filters, subtitle download, Reddit, diagnostics | `subtitleDownload` (SRT), `videoVisualFilters` (6-slider CSS filter panel), `dearrowPeekButton` (Alt-hold), `videoAgeColors`, `watchPageTabs`, `redditComments` (non-credentialed), `diagnosticLog`, `storageQuotaLRU`, `apiRetryBackoff` |
| v3.10.0 | Watch analytics, subtitle styling, AI summary, chapters | `watchHistoryAnalytics` (30-day CSS bar chart), `subtitleStyling` (caption CSS overrides), `aiVideoSummary` (BYO-key OpenAI/Anthropic/Gemini/Ollama), `copyChapterMarkdown`, `chapterJumpButtons` |
...
Research across 20+ extensions, userscripts, and open-source frontends. Projects analyzed: ImprovedTube, Enhancer for YouTube, SponsorBlock, Return YouTube Dislike, DeArrow, Unhook, BlockTube, YouTube Redux, Clickbait Remover, YouTube NonStop, competitor userscript, Tube-Insights, FreeTube, Invidious, Piped, NewPipe, SmartTube, Grayjay, yt-dlp, Tubular.

**Astra Deck's position (as of v3.10.0):** 175+ features, top 3 by feature count alongside ImprovedTube (~250 tweaks) and competitor userscript (~200 features). Consolidation of SponsorBlock, DeArrow, RYD, downloads, theater split, content controls, transcript tools, visual filters, AI summary, and watch analytics into a single extension is a strong differentiator — most users install 4-6 separate extensions to get equivalent coverage.
...
| [TimMacy/competitor userscript](https://github.com/TimMacy/competitor userscript) | N/A | AGPL-3.0 | JavaScript | 200+ features in one 568KB file proves single-file can scale but gets unwieldy. Tab view, SponsorBlock progress bar, transcript export. |
