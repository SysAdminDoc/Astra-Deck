...
function buildBaseline() {
    ...
    for (const tag of tags) {
        const keys = settingKeysAt(tag);
        const ids = featureIdsAt(tag);
    ...
    for (const key of settingKeysAt('HEAD') || []) settingKeys.add(key);
    for (const id of featureIdsAt('HEAD') || []) featureIds.add(id);
    return {
        note: 'Every setting key and feature ID that has appeared in a tagged release. Generated by scripts/generate-shipped-identity-baseline.js; it only ever grows.',
        releases: covered,
...
function main() {
    ...
    const baseline = mergeWithExisting(buildBaseline());
    ...
    if (check) {
        ...
        console.log(`[shipped-identity] OK — ${baseline.settingKeys.length} setting key(s) and ${baseline.featureIds.length} feature ID(s) across ${baseline.releases.length} release(s)`);
        return;
    }
    fs.writeFileSync(BASELINE_PATH, rendered);
    console.log(`[shipped-identity] wrote ${baseline.settingKeys.length} setting key(s) and ${baseline.featureIds.length} feature ID(s) across ${baseline.releases.length} release(s)`);
}
