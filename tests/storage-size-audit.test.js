...
test('typical local payload is not storage.sync eligible', () => {
    ...
    assert.equal(assessment.totalBytes, 176741);
...
test('storage audit report records the sync decision', () => {
    ...
    assert.match(report, /Full UI preferences payload: not viable for sync \(10\.2 KB/);
    assert.match(report, /Whole chrome\.storage\.local payload: not viable for sync \(172\.6 KB/);
