...
test('typical local payload is not storage.sync eligible', () => {
    ...
    assert.equal(assessment.totalBytes, 174030);
...
test('storage audit report records the sync decision', () => {
    ...
    assert.match(report, /UI preferences sync candidate: viable \(7\.6 KB/);
    assert.match(report, /Whole chrome\.storage\.local payload: not viable for sync \(170\.0 KB/);
