...
test('typical local payload is not storage.sync eligible', () => {
    ...
    assert.equal(assessment.totalBytes, 176508);
...
test('storage audit report records the sync decision', () => {
    ...
    assert.match(report, /Full UI preferences payload: not viable for sync \(10\.0 KB/);
    assert.match(report, /Whole chrome\.storage\.local payload: not viable for sync \(172\.4 KB/);
