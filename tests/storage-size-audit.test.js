...
test('typical local payload is not storage.sync eligible', () => {
    ...
    assert.equal(assessment.totalBytes, 177116);
...
test('storage audit report records the sync decision', () => {
    ...
    assert.match(report, /Full UI preferences payload: not viable for sync \(10\.6 KB/);
    assert.match(report, /Whole chrome\.storage\.local payload: not viable for sync \(173\.0 KB/);
