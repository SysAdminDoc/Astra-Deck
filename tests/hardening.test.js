...
test('v5.0.0 settings-schema exports the required surface', () => {
    ...
    // the opt-in scheduled selector refresh (485).
    // Keep the literal so a future schema addition must bump this
    // number deliberately.
    assert.equal(settingsSchemaModule.SETTINGS_SCHEMA.length, 485,
        'SETTINGS_SCHEMA must cover all 485 non-credential settings');
});
...
test('v4.46.0 dependency auditing is enforced by local scripts', () => {
    ...
    assert.equal(pkg.scripts['audit:deps'],
        ...
        'local npm audit must cover production and the reviewed development dependency graph');
...
test('v4.47.0 NF25 — SETTINGS_VERSION parity across ytkit.js, popup.js, and settings-meta.json', () => {
    ...
    assert.match(checkSrc, /process\.exit\(productOk && settingsOk && docsOk && tagsOk && releaseOk \? 0 : 1\)/,
        'check-versions.js must require product, settings, active-doc truth, product-tag sanity, and release currency to exit 0');
