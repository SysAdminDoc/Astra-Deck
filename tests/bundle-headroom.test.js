...
test('bundled module bodies carry no whole-line comments', () => {
    ...
    // The line scanner keeps a comment line that carries a quote, a backtick or
    // an interpolation, so a scanner mistake costs bytes rather than data. The
    // parser pass after it knows exactly which lines are comments, so none may
    // survive at all now; the old allowance let about 31 KB of them ship.
    const unexplained = commentLines
        .filter((line) => !line.startsWith('// Generated by sync-userscript.js'));
    assert.deepEqual(unexplained, [],
        'every comment in a bundled module body should have been stripped');
});
...
// The line scanner keeps any comment line that carries a quote, a backtick or
// an interpolation, because it cannot tell such a line from template data;
// about 31 KB of comments shipped that way. The parser pass can tell, so those
// go too, and the result must tokenize exactly like the input.
test('the parser pass drops every comment and leaves the code untouched', () => {
    const source = [
        'const a = 1; // it\'s a trailing note',
        '// a note with a `backtick` and a ${placeholder} in it',
        '/* a block note with a quote\'s edge */',
        'const s = `// this line is template data`;',
        'const re = /\\/\\/ not a comment/;',
        'const v = typeof/**/a;',
        'module.exports = { a, s, v, re: re.source };'
    ].join('\n');
    const stripped = sync.stripCommentsByParser(source, 'fixture.js');
