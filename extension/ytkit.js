...
(async function() {
    ...
    function _extractStreamingData(audioOnly) {
        try {
            ...
            // Fallback: combined format (lower quality but single stream)
            const combined = (sd.formats || [])
                .filter(f => f.url)
                .sort((a, b) => (b.height || 0) - (a.height || 0));
            if (combined.length > 0) {
                const best = combined[0];
                DebugManager.log('MediaDL', `Combined stream: itag=${best.itag} ${best.width}x${best.height}`);
                return { title, videoId, duration, videoUrl: best.url, videoItag: best.itag, videoMime: best.mimeType, combined: true };
            }
  ...
  <path fill="#ffffff" d="M 99.22,124.03 163.67,86.82 99.22,49.61 Z"/>
            ...
            'ytd-watch-metadata #bottom-row'),
        {
            ...
            _mountOverlay() {
                ...
                // Re-implement it on the video container inside the split overlay.
                this._clickToPauseHandler = (e) => {
                    const t = e.target;
                    // Ignore clicks on controls, overlays, or interactive elements
                    if (t.closest('.ytp-chrome-bottom, .ytp-chrome-top, .ytp-ce-element, .ytp-cards-teaser, .ytp-ad-overlay-container, .ytp-settings-menu, .ytp-popup, .ytp-paid-content-overlay, button, a, [role="button"]')) return;
                    // Accept clicks on the video element, its container, or the player area itself
                    if (!t.closest('#movie_player')) return;
                ...
                const mp = document.getElementById('movie_player');
                if (mp) mp.addEventListener('click', this._clickToPauseHandler);
                this._clickToPauseMp = mp;
            ...
            _unmount(keepClass) {
                ...
                if (this._clickToPauseMp && this._clickToPauseHandler) {
                    this._clickToPauseMp.removeEventListener('click', this._clickToPauseHandler);
                    this._clickToPauseHandler = null;
                    this._clickToPauseMp = null;
                }
                if (!keepClass) {
