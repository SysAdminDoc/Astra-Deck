(() => {
    ...
    // One shared ReDoS guard, from core/regex-safety.js. It used to live
    // here, which put it out of reach of the userscript bundle: this module
    // is deliberately extension-only, but Video Hider and the comment filter
    // ship in both. Absent means unsafe, never "compile it anyway".
    function hasUnsafeQuantifiers(pattern) {
        const guard = globalThis.YTKitCore?.hasUnsafeRegexQuantifiers;
        return typeof guard === 'function' ? guard(pattern) : true;
    }
