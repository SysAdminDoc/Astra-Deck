...
- [ ] P0 — Tag and publish the v4.60.1 release
  ...
  Note (2026-08-18, drain #2): the tree is now at **v4.66.0**, all work is committed and `origin/main` is in sync (`3043c0c3`), and all three profiles have no-CRX artifacts built at v4.64.0. The publish should target **v4.66.0**. The earlier "four unpushed commits" blocker is cleared — only the operator actions remain: tag, GitHub Release assets, and channel promotion (`release-channels.json` still points every channel at 4.59.0, five versions back).
