...
- [ ] P0 — Tag and publish the v4.60.1 release
  ...
  Note (2026-08-19, drain #10): the tree is now at **v4.74.0**, all work is committed and origin/main is in sync, and all three profiles have ephemeral-CRX validation artifacts built at v4.74.0. The publish should target **v4.74.0**. A public release still needs the external maintainer key (`ASTRA_CRX_KEY_PATH`, or `%LOCALAPPDATA%\Astra-Deck\keys\ytkit.pem`), which is absent on this machine, so the shipped CRX must be rebuilt on a machine that has it. The remaining operator actions are unchanged: tag, GitHub Release assets, and channel promotion (release-channels.json still points every channel at 4.59.0, thirteen versions back).
