...
- [ ] P0 — Tag and publish the v4.60.1 release
  ...
  Note (2026-08-20, drain #15): the tree is now at **v4.81.0**, committed and in sync with origin/main, with all three profiles built at v4.81.0 (no-CRX). The publish should target **v4.81.0**. Unchanged operator actions: tag, GitHub Release assets, and channel promotion (`release-channels.json` still points every channel at 4.59.0, twenty-two versions back).
  Note (2026-08-20, drain #14): the tree is now at **v4.80.0**, committed and in sync with origin/main, with all three profiles built at v4.80.0 (no-CRX). The publish should target **v4.80.0**. Unchanged operator actions: tag, GitHub Release assets, and channel promotion (`release-channels.json` still points every channel at 4.59.0, twenty-one versions back).
  Note (2026-08-20, drain #13): the tree is now at **v4.79.0**, committed and in sync with origin/main, with all three profiles built at v4.79.0 (no-CRX). The publish should target **v4.79.0**. Unchanged operator actions: tag, GitHub Release assets, and channel promotion (`release-channels.json` still points every channel at 4.59.0, twenty versions back).
  Note (2026-08-20, drain #12): the tree is now at **v4.78.0**, committed and in sync with origin/main, with all three profiles built at v4.78.0 (no-CRX). The publish should target **v4.78.0**. Unchanged operator actions: tag, GitHub Release assets, and channel promotion (`release-channels.json` still points every channel at 4.59.0, nineteen versions back).
  Note (2026-08-20, drain #11): the tree is now at **v4.77.0**, committed and in sync with origin/main, with all three profiles built at v4.77.0 (no-CRX). The publish should target **v4.77.0**. Unchanged operator actions: tag, GitHub Release assets, and channel promotion (`release-channels.json` still points every channel at 4.59.0, eighteen versions back). A public CRX still needs the external maintainer key, absent on this machine.
  Note (2026-08-19, drain #10): the tree is now at **v4.76.0**, all work is committed and origin/main is in sync, and all three profiles have ephemeral-CRX validation artifacts built at v4.76.0. The publish should target **v4.76.0**. A public release still needs the external maintainer key (`ASTRA_CRX_KEY_PATH`, or `%LOCALAPPDATA%\Astra-Deck\keys\ytkit.pem`), which is absent on this machine, so the shipped CRX must be rebuilt on a machine that has it. The remaining operator actions are unchanged: tag, GitHub Release assets, and channel promotion (release-channels.json still points every channel at 4.59.0, thirteen versions back).
  Note (2026-08-19, drain #8): the tree is now at **v4.70.0**, all work is committed and `origin/main` is in sync, and all three profiles have no-CRX artifacts built at v4.70.0. The publish should target **v4.70.0**. Only the operator actions remain: tag, GitHub Release assets, and channel promotion (`release-channels.json` still points every channel at 4.59.0, eleven versions back).
  Note (2026-08-19, drain #7): the tree is now at **v4.69.0**, all work is committed and `origin/main` is in sync, and all three profiles have no-CRX artifacts built at v4.69.0. The publish should target **v4.69.0**. Only the operator actions remain: tag, GitHub Release assets, and channel promotion (`release-channels.json` still points every channel at 4.59.0, ten versions back).
  Note (2026-08-18, drain #6): the tree is now at **v4.68.0**, all work is committed and `origin/main` is in sync, and all three profiles have no-CRX artifacts built at v4.68.0. The publish should target **v4.68.0**. Only the operator actions remain: tag, GitHub Release assets, and channel promotion (`release-channels.json` still points every channel at 4.59.0, nine versions back).
  Note (2026-08-18, drain #2): the tree is now at **v4.67.0**, all work is committed and `origin/main` is in sync (`3043c0c3`), and all three profiles have no-CRX artifacts built at v4.64.0. The publish should target **v4.67.0**. The earlier "four unpushed commits" blocker is cleared — only the operator actions remain: tag, GitHub Release assets, and channel promotion (`release-channels.json` still points every channel at 4.59.0, five versions back).
  Note (2026-08-18 audit): stale again — the tree is now at **v4.62.0** with FOUR unpushed commits (`d1b332ae..2b839c33`, origin/main sits at v4.61.0's `68a4aa09`) and still no tag or release past v4.59.1; `release-channels.json` channels all still point at 4.59.0. The publish should target v4.62.0, and `git push origin HEAD:main` is now also outstanding alongside the tag/release/promotion steps. A companion gate item ("Nothing gates release staleness") was logged in ROADMAP.md 2026-08-18 so this class of lag fails a check next time.
