...
| `optional_host_permissions: i.ytimg.com / returnyoutubedislikeapi.com / reddit.com` | Default-off enrichment calls for thumbnail upgrade/download, estimated Return YouTube Dislike counts, and Reddit discussion panel. Requested from the popup when the user enables the matching feature; the background proxy checks the current grant before fetching. No cookies are sent. |
