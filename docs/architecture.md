...
This document orients a new contributor to the moving parts. It is descriptive (what is, today, at v4.69.0), not prescriptive (which direction to push). Product-version sources (`package.json`, `extension/manifest.json`, `extension/ytkit.js`, `YTKit.user.js`, and `package-lock.json`) currently agree at v4.69.0. For where to push, see [ROADMAP.md](../ROADMAP.md) for the active backlog and [RESEARCH.md](../RESEARCH.md) for the current research-backed rationale. Legacy v5/v6 labels in older docs are internal planning-track names, not shipped release versions.
...
| Runtime graph | `108` modules, including `26` peeled feature modules and `291` declared feature IDs |
...
9. **The settings panel is styled in one place: `extension/core/settings-visual-system.js`.** It is the SSOT, it prefixes every selector with `#ytkit-settings-panel` and marks every declaration `!important`, and it wins. `ytkit.js` still carries several older panel stylesheets (a base layer inside `injectPanelStyles()` and a large "command center v2" correction layer below it); those are legacy and are overridden for almost every selector the v4 system covers, so editing them changes nothing you can see. Cascade order is also not source order: `injectPanelStyles()` is lazy — it runs on first panel build, so its sheets are appended *after* the eager top-level one that appears later in the file. Do not reason about panel CSS by reading it. Run `npm run probe:panel-colors`, which opens the real panel headless and reports the computed value of each surface plus the sheet that supplied it.
