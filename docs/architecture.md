...
4. **Toolbar popup**, `extension/popup.html` + `popup.js` + `popup.css`, one of three surfaces that manage settings, alongside the in-page settings panel (`extension/features/settings-panel/index.js`) and the side panel (`extension/sidepanel.html`). The standalone options page was retired in v3.19.0; the popup is the only one that is always reachable, since the other two need a YouTube tab. Settings have addresses across that split: the panel accepts a key on the fragment (`#ytkit-setting=<key>`) and on the `YTKIT_OPEN_PANEL` message, so the popup's surface chip lands on the row rather than on whatever category was last shown. The key is shape checked on both sides and matched against the card's `data-setting-key`, never interpolated into a selector.
...
background.js (MV3 service worker)
    ...
    Reveal pruner            chrome.downloads.onChanged / onErased, surfaces "Show in folder" exactly when
...
|---------|----------|-----|
| Feature settings (default values) | `extension/core/settings-schema.js` + `extension/default-settings.json` (mirror, parity-gated by `scripts/check-settings.js`) | Single source of truth for the generated schema count + metadata |
...
| Test suites | `tests/*.test.js` + `tests/features/*.test.js` (3085 JS tests, `node --test`) | Hardening regressions, parity gates, selector regression, settings migration round-trip. The companion's Python suite runs in its own repository. |
...
7. **Catch blocks need `// reason:`.** Every empty `catch (_) {}` body must explain itself with a `// reason:` comment OR contain at least one executable statement (logger, return, anything). Enforced by `scripts/eslint-rules/require-catch-reason.js`.
8. **Schema parity.** Adding a new setting requires touching BOTH `core/settings-schema.js` (the truth) AND `default-settings.json` (the mirror). `scripts/check-settings.js` enforces parity on every `npm run check`. Tests in `tests/settings-migration-roundtrip.test.js` enforce that prior-schema-version exports round-trip cleanly into the current schema.
9. **The settings panel is painted by `extension/core/settings-visual-system.js`, but it is not styled there alone.** The visual system is the SSOT *for the surfaces it covers*: it prefixes every selector with `#ytkit-settings-panel`, marks every declaration `!important`, and wins on those. It covers painting — backgrounds, borders, shadows, accent surfaces. It does **not** cover most of the panel's structure. `ytkit.js` still carries older panel stylesheets (a base layer inside `injectPanelStyles()` and a large "command center v2" correction layer below it), and `npm run probe:panel-colors --legacy-audit` measures exactly how live they are: **206 rules carrying 1,544 declarations across 3 legacy sheets set properties the visual system sets on no element they match** (measured 2026-09-05). Those declarations are the panel's typography, spacing, flex layout, badges, search states, nav metadata and feature-card structure. Deleting the legacy sheets without porting them would strip all of it. An earlier revision of this section said editing them "changes nothing you can see"; that is true only of background and border painting on the surfaces the visual system already claims, and it is why a roadmap item once proposed deleting them outright. Cascade order is also not source order: `injectPanelStyles()` is lazy, it runs on first panel build, so its sheets are appended *after* the eager top-level one that appears later in the file. Do not reason about panel CSS by reading it. Run `npm run probe:panel-colors`, which opens the real panel headless and reports the computed value of each surface plus the sheet that supplied it; add `--inventory` to identify each sheet, `--legacy-audit` to list what a deletion would have to port.
...
See [ROADMAP.md](../ROADMAP.md) for the active backlog. The current residue is mostly browser-bounded capture expansion for authenticated history, Watch Later, and open-notifications menu state, hosted security-policy mutations that must follow [docs/hosted-policy-closure.md](hosted-policy-closure.md), per-category lifecycle adoption, and the Subscription Manager v2 / Astra Downloader signing larger bets.
