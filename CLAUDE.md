...
- **Theater Split** (stickyVideo): fullscreen-aware (hides overlay, restores player, re-mounts on exit), escape key collapse, touch divider drag, double-click divider reset (75/25), dismissed state (close button/escape blocks re-expand until next video), scroll-up collapse requires 3 ticks within 600ms
...
- **SponsorBlock** fetches segments via hash-prefix privacy API (`/api/skipSegments/{prefix}`), auto-skips during playback via scheduled `setTimeout` (event-driven on `playing`/`seeked`/`ratechange`), renders colored segment bars on the progress bar. 9 category toggles (all skip-categories on by default, poi_highlight off). No skip toast — notifications were removed to avoid overlaying the video.
...
### v3.9.0 — Visual Filters, Subtitle Download, Reddit, Diagnostics
- `subtitleDownload`: one-click SRT download via player button. Reuses JSON3 caption fetch, file = `${videoId}_${lang}.srt`.
- `videoVisualFilters`: 6-slider CSS filter panel (brightness/contrast/saturation/hue/grayscale/sepia) on `.html5-main-video`. Catppuccin panel anchored to a player-controls button. Persists via setting keys `vvf*`.
- `dearrowPeekButton`: hold Alt → CSS overlay reveals original titles. Listens to keydown/keyup + window blur.
- `videoAgeColors`: CSS pseudo-element border on thumbnail cards by upload-age bucket (fresh/week/month/year/ancient). Mutation + navigate rule.
- `watchPageTabs`: injects Description/Comments/Chapters/Transcript tab bar above `#below.ytd-watch-flexy`. One section visible at a time.
- `redditComments`: secondary-sidebar panel, fetches `reddit.com/search.json?q=url:youtube.com/watch?v={id}` via EXT_FETCH. Reddit added to `ALLOWED_FETCH_ORIGINS` only (no credentials).
- `diagnosticLog`: 500-entry ring buffer, hooks console.error + window.onerror when ytkit-tagged. `window.__ytkitDiagnostics.download()` emits JSON bug report.
- `storageQuotaLRU`: 5-min periodic LRU sweep of hiddenVideos(5k), hiddenChannels(2k), timestampBookmarks(2k), deArrowCache(1k), `_errors`(500).
- `apiRetryBackoff`: new `extensionRequestWithRetry()` helper wrapping `extensionRequestAsync`; `extensionFetchJson` now retries 1s/2s/4s on 429/5xx/network errors. Default ON.
- New settings: `subtitleDownload`, `videoVisualFilters`, `vvfBrightness`, `vvfContrast`, `vvfSaturation`, `vvfHue`, `vvfGrayscale`, `vvfSepia`, `dearrowPeekButton`, `videoAgeColors`, `watchPageTabs`, `redditComments`, `diagnosticLog`, `_errors`, `storageQuotaLRU`, `apiRetryBackoff`.

## Previous Version: v3.8.0

### v3.8.0 — Toolbar Popup, Digital Wellbeing, Profiles
- Toolbar popup (`popup.html` / `popup.js` / `popup.css`, Catppuccin Mocha) — 15 quick toggles, live filter, broadcast via `YTKIT_SETTING_CHANGED` for no-reload updates.
- `manifest.json` now has `action.default_popup: "popup.html"` — `chrome.action.onClicked` no longer fires; `Ctrl+Shift+Y` still toggles in-page panel.
- `settingsProfiles`: save/load/delete/export/import JSON. State: `_profiles`, `_activeProfile`. Exposed as `window.__ytkitProfiles`. Schema v1.
- `digitalWellbeing`: break reminders + daily cap. Ticker in content script (not SW). Persists `dwWatchTimeToday`.
- `videoRotation`: 90/180/270° via CSS transform + 0.5625 scale for 90/270° to fit 16:9 box.
- `frameByFrameButtons`: ⏮/⏭ in `.ytp-right-controls`, 1/30s step, auto-pause.
- New content-script message handler: `YTKIT_SETTING_CHANGED` → re-init/destroy matching feature.
- New settings: `videoRotationAngle`, `frameByFrameButtons`, `digitalWellbeing`, `dwBreakIntervalMin`, `dwDailyCapMin`, `dwWatchTimeToday`, `_profiles`, `_activeProfile`.

## Older: v3.7.0

### v3.7.0 — Transcript Export, Privacy Hardening
- Transcript sidebar export buttons (Copy / .txt / .srt / LLM prompt) reuse the JSON3 caption fetch — no extra requests.
- `cleanShareUrls` STRIP_PARAMS expanded with UTM family + click-tracking IDs (gclid, fbclid, mc_cid, mc_eid, igshid, twclid, yclid).
- Security: `background.js` EXT_FETCH proxy now defaults to `credentials: 'omit'`. Cookies only sent to YouTube-family origins + local MediaDL via `CREDENTIALED_FETCH_ORIGINS` allowlist.

### Already-Implemented Roadmap Items (audit findings)
- SponsorBlock seekbar segments: live in `_renderBarSegments` / `.ytkit-sb-segment` since pre-v3.7.
- Basic URL tracking strip: existed pre-v3.7, expanded in this release.
- Settings Profiles: feature stub still present — no implementation yet (roadmap v3.7.x backlog).
- Toolbar popup: not built; toolbar action still toggles in-page panel via `togglePanelForTab`.
- Break reminders / watch-time limits: not built yet (roadmap v3.7.x backlog).
